...
4. Часть маппингов **однозначна** (fixed decision в ADR-001/002), часть —
   **неоднозначна** и вынесена на человеческое решение (`frameworks/`, `guides/`,
   `education/`, `experiments/`, `website/`, `docs/concept.md`,
   `projects-sink/AI_PROJECT_CONTEXT-Summary.md`, граница `practices/` ↔ `docs/practice/`,
...
| `docs/` (`adr/`, `analysis/`, `audit/`, `report/`, `rfc?`) | — | ADR, Analysis, Audit-report, Report. `docs/rfc/` пока нет (RFC в `docs/rfc/`). |
...
| root-файлы | 10 | `README.md`, `CHANGELOG.md`, `CONTRIBUTING.md`, `LICENSE`, `AI_GOVERNANCE.md`, `projects-sink/AI_PROJECT_CONTEXT-Summary.md`, `docs/concept.md`, `mkdocs.yml`, `.gitignore`, `.gitkeep`. |
...
| `docs/rfc/` | Proposal до решения | `docs/rfc/` (Хаб) | 🟠 Мигрировать `docs/rfc/` → `docs/rfc/` |
...
| `docs/concept.md` (root) | Концептуальный документ | `docs/` или `research/`? Тип и дом на решение |
| `projects-sink/AI_PROJECT_CONTEXT-Summary.md` (root) | Сводка контекста проекта | `kb/` (operational) или `docs/`? |
...
| Root-документы | `docs/concept.md`, `projects-sink/AI_PROJECT_CONTEXT-Summary.md` | `docs/` / `research/` / `kb/` | Нет fixed decision | 🟡 Human decision |
...
3. **Фаза 2 — RFC-переезд.** `docs/rfc/` → `docs/rfc/` с картой перехода и
...
3. **Алиасы/редиректы на переходный период.** Пока `docs/rfc/` и
