...
> **Статус: проект (draft).** Это RFC — *дизайн-предложение*, а не рабочая
> инструкция. Документ анализирует инцидент «анализ вместо исполнения», сравнивает
> три равноприоритетных видения с практиками индустрии и формирует **Видение 3** —
> оптимальную архитектуру исполнимых документов Хаба и шаблонов споков. Решение об
> утверждении стандарта и о физическом рефакторинге контрактов остаётся за
> человеком (см. раздел 7).
...
предполётного чек-листа и модель процесса — здесь и в RFC `proposals/`.
...
## 7. Запрос на согласование

Перед любым физическим изменением контрактов нужно твоё решение, Фаундер:

1. **Утверждаем ли паттерн «Директива сверху, контекст под катом» (Видение 3)** как
   стандарт исполнимых документов Хаба и шаблонов споков?
2. **Принимаем ли имя машинного маркера `executable: true|false`** (с опциональным
   `entrypoint: true`) — вместо `mode:`/`execution:` из Видения 2, чтобы избежать
   коллизии с термином *Operating Mode*?
3. **Согласуем ли план внедрения** (раздел 6) и порядок приоритетов P0→P3 — серией
   малых PR, по одной задаче на файл?
4. **Размещение этого RFC.** Issue явно требует путь
   `docs/analysis/contract-executability-rfc.md`, и файл создан именно там. При этом
   репозиторная конвенция держит RFC в `governance/proposals/` (см.
   [ARTIFACT_MAP](../../governance/ARTIFACT_MAP.md)). Оставляем в `docs/analysis/`
   как явно запрошено, или переносим в `governance/proposals/` для единообразия?
   Решение за тобой.

### Что я НЕ делал без твоего слова (Anti-Inflation, границы задачи)

- ❌ Не менял физически ни один контракт (`AGENT_ONBOARDING.md`, `AI_GOVERNANCE.md`,
  `README.md`, шаблоны споков) — только *предложил* рефакторинг в примерах.
- ❌ Не добавлял термины в `GLOSSARY.md` и не правил `ARTIFACT_MAP.md` — это пункты
  плана внедрения, ждущие апрува.
- ❌ Не вводил новые поля frontmatter в существующие файлы и не менял
  `tools/validate-frontmatter.sh`.
- ✅ Создал **только один новый файл** — этот RFC — и зарегистрировал его в
  `tools/validate-repository-structure.sh` как active-артефакт (требование DoD,
  чтобы структурный валидатор проходил), не трогая содержание контрактов.
