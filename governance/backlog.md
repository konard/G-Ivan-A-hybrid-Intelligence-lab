...
| **B-016** | RFC: Структура research, контейнер `exp/` и маршрутизация Research/Analysis/Audit | **P0** | — | TODO | — (planned) | Issue [#294](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/294); issue [#290](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/290); issue [#288](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/288) | Запускает согласованную цепочку Research → RFC → ADR → Standard и снимает текущую неоднозначность `exp-*`/`outputs` vs `runs/`. |
...
**Ожидаемые артефакты:**
- `governance/rfc/YYYY-MM-DD-rfc-research-structure.md` (новый RFC)

**Критерии приёмки (DoD):**
- [ ] RFC ссылается на issues #294, #290, #288 и связанные артефакты.
- [ ] RFC явно описывает `exp/`, запрет `outputs/` в research experiment corpus
      и границу с `runs/`.
- [ ] RFC определяет routing Research / Analysis / Audit по типу задачи, а не
      только по имени каталога.
- [ ] RFC указывает последствия для B-017..B-023.
