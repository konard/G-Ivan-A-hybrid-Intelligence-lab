...
temperature: 0.1
type: backlog
context: [governance, backlog, sprint-3, creative-analysis, prioritization, hub-and-spoke, executable-documents, research-structure, analysis-standardization, audit-standardization, reports-standardization]
...
related_issues:
  ...
  - "https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/340"
  - "https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/342"
  - "https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/344"
  - "https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/350"
  - "https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/354"
  - "https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/357"
  - "https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/358"
  - "https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/362"
  - "https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/364"
...
| **B-004** | Зафиксировать двухкейсовую модель инициализации в `governance/repo-model.md` | **P1** | — | DONE (artifact: canonical) | [#111](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/111) | RFC-два-кейса (follow-up #2) | Закрепляет концептуальный фундамент в каноне; снижает риск повторной терминологической путаницы (ошибка №5 ретроспективы). |
...
| **B-042** | adr: Принятие структуры Reports + реконсиляция routing ADR-002 (`docs/reports/` → `docs/report/`) | **P1** | B-041 | DONE (artifact: accepted) | [#338](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/338) (PR [#339](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/pull/339)) | Issue [#338](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/338); RFC B-041 ([governance/rfc/2026-07-02-rfc-reports-structure.md](rfc/2026-07-02-rfc-reports-structure.md)); [ADR-004](../docs/adr/2026-07-adr-004-reports-structure.md); [ADR-002](../docs/adr/2026-06-adr-002-artifact-document-methodology.md); `standards/adr-structure-standard.md` | Human decision gate выполнен: принят Вариант C и канонический `docs/report/`, строка routing ADR-002 (`docs/reports/` → `docs/report/`) реконсилирована как single decision source перед нормативным стандартом. Зеркалит B-026/B-031. |
...
| **B-046** | chore: Синхронизировать `governance/backlog.md` с фактическими статусами артефактов | **P1** | — | review | [#364](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/364) (PR [#365](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/pull/365)) | Issue [#364](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/364); artifact frontmatter statuses; this backlog | Поддерживает canonical backlog как актуальный источник задач: закрытые issues/merged PR сверены с lifecycle-статусами артефактов, без миграции файлов и без изменения внешних артефактов. |
...
| --- | --- | --- | --- | --- | --- |
| CE-001 | `governance/agent-onboarding-protocol.md` | P0 | CE-008 | DONE (artifact: canonical) | [#138](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/138) |
...
| CE-005 | `governance/repo-model.md` | P2 | CE-008 | DONE (artifact: canonical) | [#142](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/142) |
...
| CE-010 | `governance/artifact-map.md` | P2 | CE-001, CE-002, CE-003, CE-004, CE-008 | DONE (artifact: canonical) | [#147](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/147) |
...
`governance/agent-onboarding-protocol.md`; он создан и имеет lifecycle-статус
...
**Что сделано:**
1. Создан `governance/agent-onboarding-protocol.md` по дизайну онбординг-RFC:
...
**Ожидаемые артефакты:**
- `governance/agent-onboarding-protocol.md` (создан, status `canonical`)
...
**Что сделано:**
1. В `README.md` добавлен блок «Новый агент? Начни здесь → `governance/agent-onboarding-protocol.md`».
...
**Что было сделано:**
1. Создан `governance/rfc/2026-07-02-rfc-analysis-structure.md` (текущий status
...
будущий ADR B-026
**Зависимости:** B-026
**Статус:** review (PR [#369](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/pull/369))
**Режим работы:** `Structured`
...
**Что сделано:**
1. Создан `governance/rfc/2026-07-02-rfc-audit-structure.md`.
...
- **Внешние источники** `[GAP]`, `[EGA]`, `[AID]` не верифицированы независимо;
  ...
  external-review).
- **Статусы отражают факт на 2026-07-03.** Пересмотр — по триггерам раздела 7.
...
Диаграмма фокусируется на историческом critical path B-001..B-034; строки
B-035..B-046 ведутся в сводной таблице раздела 2. B-001..B-008, B-010, B-011,
B-013, B-016..B-019, B-021, B-024..B-026 и B-029..B-032 уже выполнены.
Runtime-онбординг и связанная документация Кейса 2 закрыты через
`B-001 → B-002` и `B-001 → B-005`. Sprint issue #294 оставляет открытыми
glossary/validator cleanup задачи B-020, B-022 и B-023.

Issue #296 ведёт два параллельных sprint chains:
`B-020 → B-024 → B-025 → B-026 → B-027 → B-028` для Analysis и
`B-020 → B-029 → B-030 → B-031 → B-032 → B-033` для Audit. Audit chain дошла до
созданного `standards/audit-standard.md`; Analysis chain создала
`standards/analysis-standard.md` (B-027, review).
План физической реструктуризации Хаба остаётся отдельным RFC B-034 после
появления всех трёх standard artifacts: `B-018`, `B-027` и `B-032`.

```mermaid
flowchart TD
    B010["B-010 · DONE<br/>Сняты лимиты промптов<br/>(prompt body не менялся)"]
    B001["B-001 · DONE<br/>agent-onboarding-protocol.md<br/>(Кейс 1 + threat awareness)"]
    B002["B-002 · DONE<br/>Ссылки из README<br/>и AI_GOVERNANCE"]
    B005["B-005 · DONE<br/>spoke/README (Кейс 2)<br/>+ перекрёстные ссылки"]

    B003["B-003 · DONE<br/>Handover Prompt<br/>в templates/htom/"]
    B004["B-004 · DONE<br/>Двухкейсовая модель<br/>в repo-model.md"]
    B006["B-006 · DONE<br/>Fail-closed<br/>→ AI_QUICK_RULES"]
    B007["B-007 · DONE<br/>Capability taxonomy<br/>→ AI_GOVERNANCE"]
    B011["B-011 · DONE<br/>Evidence model<br/>назван явно"]
    B008["B-008 · DONE<br/>research/ hub+mango"]
    B013["B-013 · DONE<br/>Промоут бэклога<br/>→ issues по P1"]
    ...
    B021["B-021 · DONE<br/>remove research-profile.md<br/>after replacement"]
    B022["B-022 · P2<br/>migrate exp-*<br/>remove outputs/"]
    B023["B-023 · P2<br/>validator support<br/>exp/ + routing"]
    B024["B-024 · DONE<br/>Analysis artifacts<br/>cross-repo inventory"]
    B025["B-025 · DONE<br/>RFC analysis-standard<br/>structure + routing"]
    B026["B-026 · DONE<br/>ADR-006 analysis structure<br/>human decision"]
    B027["B-027 · review<br/>analysis-standard.md<br/>structure standard"]
    B028["B-028 · P2<br/>Analysis cleanup<br/>modernization"]
    B029["B-029 · DONE<br/>Audit artifacts<br/>cross-repo inventory"]
    B030["B-030 · DONE<br/>RFC audit-standard<br/>structure + routing"]
    B031["B-031 · DONE<br/>ADR audit-standard<br/>human decision"]
    ...
    class B027,B034 crit;
    class B014,B015,B020,B022,B023,B028,B033 norm;
    class B001,B002,B003,B004,B005,B006,B007,B008,B010,B011,B013,B016,B017,B018,B019,B021,B024,B025,B026,B029,B030,B031,B032 done;
```

Чтение диаграммы: зелёные узлы уже выполнены; жёлтые узлы — оставшиеся critical
gates для Analysis standard и downstream migration-RFC; голубые — параллельные
или post-standard cleanup tasks. Пунктирные стрелки — мягкие зависимости
(порядок/апрув), сплошные — жёсткие.
