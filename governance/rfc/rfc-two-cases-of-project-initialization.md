...
| №1: «operating mode должен быть проверяемым значением» | Смешанный кейс провоцирует смешанный режим («инициализация» = и Structured, и Project) | Манифест жёстко привязывает Кейс 1 → `Structured`, Кейс 2 → `Project` |
...
1. **Уточнить `rfc-agent-onboarding-protocol.md`** — добавить раздел «Модель
