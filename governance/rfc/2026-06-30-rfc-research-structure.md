...
| Decision record | [ADR-003](../../docs/adr/2026-07-adr-003-research-structure.md) (B-017, accepted) |
...
Предлагается единая базовая модель структуры research-артефактов Хаба:
...
Это RFC, а не норма. Он стал входом для ADR-003 (B-017) и стандарта
...
Почему текста issue/PR недостаточно: решение меняет публичное правило структуры
Хаба и затрагивает downstream-цепочку B-017..B-023. Такое изменение
...
| --- | --- | --- |
| A1 | Сохранить sibling `exp-<slug>/` с `outputs/` (рекомендация аудита №3). | Не снимает коллизию `outputs/` ↔ `runs/`; оставляет ambiguous future structure rule и засоряет корень направления смесью отчётов и папок. |
...
Этот RFC имеет `rfc-scope: A`, потому что меняет базовое правило структуры Хаба. Матрица
...
| --- | --- | --- |
| A. Governance & Knowledge Hub | Принять или отклонить целевую структуру `research/<domain>/exp/<issue-slug>/`, запрет `outputs/`, routing Research / Analysis / Audit и границу `exp/` vs `runs/` через ADR B-017, стандарт B-018 и addendum B-019. | Не выполнять миграцию `exp-*`, не менять валидаторы research-формата и не превращать RFC в норму до human decision. |
...
| Переходный dual-model порождает новый дрейф. | Сосуществование форматов — риск второй коллизии. | Legacy формат заморожен и читается однозначно; целевой формат применяется к новой работе после B-018; миграция — отдельной задачей B-022 после стандарта. Дрейф ограничен по времени и зафиксирован в backlog. Принято. |
...
| B-022 | Миграция `exp-*` → `exp/`, удаление `outputs/` | Физическая миграция legacy по P6 после стандарта. |
| B-023 | Обновление валидаторов под `exp/` и routing | Делает стандарт исполнимым после human decision. |
...
Текущее состояние: `accepted`. Переход выполнен по ADR-003
([issue #322](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/322) /
PR [#323](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/pull/323)).
