...
| Standard meta-structure | Proposed | F10 explicit для **всех стандартов экосистемы** (не только Research/Analysis/Audit/Report): десять инвариантных секций в строгом порядке, `N/A + rationale`, `Type Model` = `model`/`N/A`, specific tail с Purpose/Scope cross-reference и validation boundary | [standard-meta-structure.md](standard-meta-structure.md) |
...
| Artifact deprecation standard | Draft (предложен на утверждение) | Ортогональная ADR-002 ось Deprecation для артефакта во времени (issue #500): состояния `active`/`deprecated`/`superseded`/`archived`/`deleted`, легитимные и запрещённые переходы, механизмы совместимости (compatibility redirect, superseding record, validator allowlist, historical link policy), preconditions физического удаления и классификация составного перехода целиком по максимальному весу | [artifact-deprecation-standard.md](artifact-deprecation-standard.md) |
| Product taxonomy reference | Draft (предложен на утверждение) | Чистый справочный артефакт Хаба по продуктовой таксономии (issue #563): цепочка наследования Industry (L0) → IT/Telecom (L1) → продуктовая линейка (L2/L3) и иерархия `Domain → Capability → Feature → Atomic Function`, унаследованная от `mango_ba_prompts/standards/product-classification-contract.md`. Форма уровней и правила отнесения без операционной информации: каталог значений остаётся в спице | [product-taxonomy-reference.md](product-taxonomy-reference.md) |
