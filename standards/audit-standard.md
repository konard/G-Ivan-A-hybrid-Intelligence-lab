...
Audit-процесс vs audit-report output (см. [Type Model](#type-model) и
[Audit-процесс vs audit-report output](#audit-процесс-vs-audit-report-output)).
...
`model`. Модель типа Audit реализует **Вариант C** ADR-005: один базовый стандарт
Audit + явная 4-компонентная модель процесса. Audit ОПРЕДЕЛЯЕТСЯ связкой из
четырёх компонентов, а не путём `docs/audit/` и не именем файла. Это самое
стабильное ядро из
...
[Boundaries](#boundaries)). Отдельная таблица subtype profiles Audit не вводится:
единственная форма типа — эта 4-компонентная модель.
...
дублирует и не переопределяет её. Ниже — только локальная delta Audit: граничные
кейсы, где стойку определяет 4-компонентная модель.
...
- [ADR-002: Методология создания и управления артефактами](../docs/adr/2026-06-adr-002-artifact-document-methodology.md) —
  routing и knowledge-lifecycle артефактов, канонический `docs/audit/`; canonical
  owner общей artifact boundary/routing table.
...
Этот раздел реализует границу Audit ↔ Report, заявленную в [`Scope`](#scope), и
проекцию **Вариант C** из [`Purpose`](#purpose): разделение процессной семантики
Audit и формы её durable output. Audit-цепочка и Reports-цепочка координируются,
но НЕ подменяют друг друга (ADR-005, ADR-004 v0.3):
