...
| Governance | `docs/rfc/`, `docs/rfc/`, `docs/adr/`, `standards/`, `ai-rules/` | `draft`, `proposed`, `accepted`, `rejected`, `deprecated`, `superseded` | Decision process: drafted, proposed for review, accepted/rejected, retired or replaced. |
...
| RFC | `docs/rfc/*.md`, `docs/rfc/*.md` | `status`, `version`, `updated`, `temperature`, `owner`, `rfc-scope` | `executable`, `entrypoint`, and legacy research-routing fields only when consumed. |
...
| `entrypoint: true` | Artifact must be listed from a known navigation entry such as `README.md`, `GOVERNANCE.md`, or `ops/artifact-map.md`. |
