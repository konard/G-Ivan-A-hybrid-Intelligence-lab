...
| ID | Название | Приоритет | Зависимости | Статус | Issue | Источник | Краткое содержание | Режим запуска |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **B-064** | Довести `task.yml` до модели: сделать Hybrid и Deep Think выбираемыми | **P1** | - | TODO | - (planned) | Global analysis v0.4 §8/R1; §11-B1; H3; `research/hub/2026-07-04-hub-as-agent-system-global-analysis.md` | Форма задачи расходится с моделью в несущем свойстве «режим = регулятор автономии»: glossary знает Hybrid/Deep Think, а `task.yml` их не предлагает. Сделать их выбираемыми или явно задокументировать, почему их нет. | `null` |
| **B-065** | Зафиксировать метод стресс-тестирования как повторяемую процедуру | **P1** | - | TODO | - (planned) | Global analysis v0.4 §8/R3; §9.4; §11-C1; H4/H27 | Метод применён ≥2 раза (этот отчёт + cross-standard-report), но не нормирован: нет единого словаря вердиктов, критерия достаточности тестов и route вывода. Обобщает §9.4; B-054 — отложенное частное применение к связанным документам. | `null` |
| **B-066** | Единый OWASP-LLM чек-лист агентной системы (закрывает R2/G2) | **P1** | - | TODO | - (planned) | Global analysis v0.4 §15.8.2/RFC-D; §15.1.8; §3.3; H25; §11-G2/R2 | Security-материал уже есть в практиках; отсутствует точка сборки с трассой покрытия по OWASP LLM Top-10:2025 и SAIF. Задача поглощает рекомендацию R2 (тот же пробел G2), поэтому R2 отдельной строкой не заводится. | `null` |
