...
| ID | Название | Приоритет | Зависимости | Статус | Issue | Источник | Краткое содержание | Режим запуска |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **B-022** | Мигрировать существующие `exp-*` в контейнер `exp/`, убрать `outputs/` | **P2** | B-018, B-019 | review | [#439](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/439) | Issue #294; issue #290; текущие `research/hub/exp-*` | Legacy experiment folders перенесены в `research/hub/exp/<issue-slug>/`, evidence flattened без изменения содержательных результатов. | Structured |
| **B-023** | Обновить валидатор структуры под `exp/` и routing по типам задач | **P2** | B-018, B-019 | review | [#439](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/439) | Issue #294; `tools/validate-repository-structure.sh`; `tools/validate-file-naming.sh` | Добавлен исполнимый `exp/` contract: запрет legacy shape и `outputs/`, проверка README и parent links с regression tests. | Structured |
| **B-028** | Cleanup и модернизация Analysis-артефактов | **P2** | B-027 | review | [#439](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/439) | Issue #296; B-024 inventory; `standards/analysis-standard.md` | Analysis corpus проверен: knowledge lifecycle корректен, явных дублей нет; missing `scope` добавлен без изменения содержательной части, cross-references синхронизированы. | Structured |
| **B-044** | Cleanup и модернизация Reports-артефактов в `docs/report/` | **P2** | B-043, B-034 | review | [#439](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/439) | Issue #328; Reports inventory; `standards/report-standard.md`; B-034 | Reports corpus проверен: добавлены `report-subtype`, `scope`, `based_on` и standard relations; явных дублей и unmasked sensitive data не найдено. | Structured |
...
| **B-052** | Создать мета-стандарт структуры стандартов | **P1** | B-051 | review | [#423](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/423), [#435](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/435) | Issue #374; [ADR-008](../docs/adr/2026-07-adr-008-standard-meta-structure.md); [Standard Meta-Structure Standard](../standards/standard-meta-structure.md) | F10 explicit нормативно зафиксирован: десять уникальных секций в строгом порядке, `N/A + rationale`, `Type Model` = `model`/`N/A`, specific-tail cross-reference, ADR-002 как canonical boundary owner и точная граница machine/human validation. Issue #435 расширил область с R/A/A/Report на **все стандарты экосистемы** (ADR-008 v0.2, решение фаундера 2026-07-16) и закрыл вопрос Q5 `evals-contract-standard.md`; мета-стандарт v0.2 объявил собственную форму `Type Model` = `model`. | Structured |
| **B-053** | Привести Research/Analysis/Audit/Report standards к мета-стандарту | **P1** | B-052 | TODO | - (planned) | Issue #374; [Standard Meta-Structure Standard](../standards/standard-meta-structure.md) | Обновить четыре стандарта двумя reviewable слоями: mechanical block moves и отдельно обоснованные semantic deltas. Синхронизировать validators/navigation where required. | Structured |
...
| ID | Название | Приоритет | Зависимости | Статус | Issue | Источник | Краткое содержание | Режим запуска |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **B-079** | ADR о разделении Mango на два репозитория | **P1** | - | review | [#424](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/424) / [PR #429](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/pull/429) | Обсуждение 2026-07-10 (решение фаундера); issues #411, #413 | Зафиксировать решение фаундера о разделении Mango на публичный (`ai-ba-playbooks`) и приватный (`mango-ba-prompt-library`) репозитории через ADR: причина разделения, структура обоих репо, механизм синхронизации, связь с Хабом. Результат: `docs/adr/2026-07-adr-009-mango-repo-split.md` (`status: proposed` до acceptance gate на merge). | Hybrid |
| **B-080** | План миграции Mango | **P2** | B-079 | review | [#436](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/436) | ADR-009 (B-079); issues #411, #413, #446; снимок `mango_ba_prompts@295b65d` | На основе ADR-009 составлен детальный план миграции: `docs/analysis/2026-07-17-mango-artifacts-migration-plan.md`. Построен на фактическом снимке (5 346 файлов, 1 337 с брендом «Mango»), а не на концептуальном списке: правила классификации R1–R8 (fail-safe по умолчанию), процесс обезличивания A1–A8 с обязательным gate до первого push, таблица миграции по всем корневым каталогам, расщепление глоссария как стратегия пограничного кейса, фазы 0–5, риски. Q2 закрыт issue #446: всё дерево `kb/` переносится в приватный репозиторий целиком; блокирующим остаётся Q1 о фактической публичности исходного репозитория. B-083 не стартует до закрытия Q1. | Hybrid |
...
| **B-082** | Создание приватного репо `mango-ba-prompt-library` | **P2** | B-079, B-080 | TODO | - (planned) | ADR B-079; план миграции B-080; issues #411, #413, #446 | Создать приватный репозиторий `mango-ba-prompt-library` со структурой операционки (`prompts/`, полное дерево `kb/` с `processed/`, `sources/`, `fragments/`, `practices/`, а также `runs/`, `evals/`, `internal-rfc/`, `internal-docs/`) и настроить локальные валидаторы без GitHub Actions. | Structured |
...
| ID | Название | Приоритет | Зависимости | Статус | Issue | Источник | Краткое содержание | Режим запуска |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **B-085** | Исследование retrieval-стратегий для AI-агентов | **P2** | - | review | [#418](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/418) | Issue #418 (постановка фаундера, гипотезы H1–H7); глобальный анализ v0.4 §15.1.4, §15.1.6, §15.2.5 (RFC-F) | Научно-теоретический survey и критический разбор retrieval-стратегий в трёх этапах: research questions, объектная модель, граф зависимостей, 9 точек решений, 20 гипотез с ранжированием; таксономии БЗ/запросов/стратегий, матрица «БЗ × Запрос → Стратегия», chunking/embeddings/reranking/evaluation, уровни готовности БЗ L0–L5; индустриальные кейсы, тренды 2024–2026, связь с памятью агента, вход для RFC-F. Результат: `research/education/2026-07-16-retrieval-strategies-survey.md` (`status: draft` до подтверждения фаундером). | Creative |
...
| **B-089** | Реорганизация исследования retrieval в Reference Research Pattern | **P2** | B-085 | review | [#437](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/437) | Issue #437 (постановка фаундера, Режим Hybrid); монолит B-085 | Структурный рефакторинг без потери содержания: монолит `research/education/2026-07-16-retrieval-strategies-survey.md` (1854 строки, `status: superseded`) разложен в модуль `research/ai-education/retrieval/` из шести файлов — `00-introduction` (BLUF, карта чтения по аудиториям, граф зависимостей разделов), `10-theory` (Conceptual Framing с сигнатурой `S = Decision(KB, Query, Constraints)`, E1–E11, D1–D9, H1–H20), `20-taxonomy` (K1–K6, Q1–Q10, S0–S6), `30-decision-framework` (матрица, chunking, embeddings, reranking, evaluation, L0–L5), `40-practice-and-cases` (кейсы, тренды, память агента, ДНК), `50-open-research` (зависимости выбора, вход для RFC-F, ранжирование гипотез, глоссарий, источники, самоаудит). Внутренние `§`-ссылки заменены на cross-references между файлами; ветка «Практика» явно опирается на «Теорию» и рамку решений. Паттерн предложен в `docs/rfc/2026-07-17-rfc-reference-research-pattern.md` (`draft`, статус паттерна — Experimental, критерий повышения — ≥3 независимых домена). Governance-решение не принимается — за фаундером (Rule 4). | Hybrid |

---

## Спринт 9: Governance и юридическая чистота

**Story.**
Корневой `LICENSE` был governance placeholder: он фиксировал, что финальная
лицензия не выбрана, и не давал пользователям прав сверх платформенных. Для
базы знаний это блокирующая неопределённость — методологии Хаба создаются для
переиспользования внешними споками и сообществом, но без лицензии внешний
пользователь юридически не может ни адаптировать стандарт, ни встроить его в
коммерческий процесс.

Дополнительная сложность в том, что репозиторий неоднороден: основной актив —
текстовые артефакты (стандарты, промпты, ADR/RFC, исследования), а скрипты
(`tools/`, CI) вторичны и требуют другой правовой рамки, включая патентный
грант. Одна универсальная лицензия покрывает один из двух типов плохо, поэтому
решение фаундера (обсуждение 2026-07-10) — гибридная модель CC BY 4.0 +
Apache-2.0.

**Цель.**
Заменить license placeholder на юридически определённую модель лицензирования,
сохраняющую обязательное указание авторства и дающую внешним пользователям
явное право на переиспользование и коммерческое применение методологий Хаба.

**Критерий закрытия.**
Спринт закрывается, когда B-085 merged: корневой `LICENSE` содержит выбранную
модель, `README.md` объясняет права человекочитаемо, а карта артефактов и
`CHANGELOG.md` синхронизированы.
...
| Issue [#437](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/437) (постановка фаундера, Режим Hybrid) | Источник задачи B-089 (Спринт 9): реорганизация монолита B-085 в модуль `research/ai-education/retrieval/` по Reference Research Pattern и RFC-предложение самого паттерна (`draft`, Experimental). |
