...
Объём этого файла нормируется в токенах: свыше 4K токенов объёмный блок выносится в суб-контракт
со ссылкой, свыше 8K токенов валидатор даёт FAIL. Блоки <hard_rules> и <forbidden> выносу не подлежат.
</scope>
...
- ЗАПРЕЩЕНО добавлять поимённые запреты каталогов в валидаторы и в этот перечень. Каталог вне
  канонического набора легализуется декларацией в `.hub-profile.json` (`project_specific_directories`:
  `path` + обязательный непустой `reason`). Отсутствие каталога в каноническом наборе не равно запрету.
