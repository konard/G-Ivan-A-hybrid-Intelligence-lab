...
- Хронологические документы в `docs/analysis/` именуются с точной датой в
  начале (`YYYY-MM-DD-name.md`), `docs/rfc/` допускает `YYYY-MM-name.md` или
  `YYYY-name.md`, ADR использует `YYYY-MM-adr-NNN-name.md`; всё проверяется через
