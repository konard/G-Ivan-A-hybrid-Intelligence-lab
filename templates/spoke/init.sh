#!/usr/bin/env bash
#
# init.sh — инициализация спока из «ДНК-шаблона» Хаба `hybrid-Intelligence-lab`.
#
# Креативное улучшение UX инициализации: один запуск заменяет плейсхолдеры во
# всех файлах шаблона и проставляет дату, чтобы новый spoke рождался уже
# «правильным», без ручного поиска {{...}} по дереву.
#
...
#   ./init.sh --name "My Spoke"                # частично через флаги
#   ./init.sh -n "My Spoke" -d "..." -u "URL" --yes   # неинтерактивно (для CI)
#
set -euo pipefail
...
printf '\n✅ Инициализирован spoke «%s» (%d файл(ов) обновлено, дата %s).\n' \
  "$project_name" "$changed" "$today"
printf '   Хаб: %s\n' "$hub_url"

# init.sh — одноразовый: после инициализации он не нужен в споке.
remove_self() {
