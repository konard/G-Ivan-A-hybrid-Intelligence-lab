...
| **Кейс 2 — *Bootstrap-клонирование*** | Из шаблонов Хаба рождается новый репозиторий: геном `templates/htom/` — для HTOM-команды (гибридная human + AI работа), `templates/spoke/` — для production-спока (код с собственным жизненным циклом). | Постоянный — однократный акт при создании. | Задаётся задачей: `Structured`, `Creative` или `Hybrid` |
