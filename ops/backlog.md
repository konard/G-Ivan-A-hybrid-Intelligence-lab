...
| **B-154** | Provenance ИИ-участия во frontmatter: решение по метке `ai-generated` | **P2** | B-116 | todo | [#569](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/569) | [Журнал решений, предложение №9](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/blob/main/docs/analysis/2026-09-10-agents-md-hybrid-work-contract-analysis.md) | Владелец репозитория предложил разрешить `ai-generated` при условии, что frontmatter явно называет людей-участников (команды, роли) и конкретную модель ИИ. Предложение **не реализовано** в PR [#570](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/pull/570): запрет записан минимум в восьми стандартах (`frontmatter-standard`, `frontmatter-docs-standard`, `analysis-standard`, `research-standard`, `report-standard`, `audit-standard`, `adr-structure-standard`, `rfc-structure-standard`), а также в `standard-meta-structure` и `CONTRIBUTING.md`, поэтому правка одного `AGENTS.md` расколола бы геном. Решить одним согласованным изменением, выбрав вариант: (A) оставить запрет — provenance фиксируется в issue, PR и changelog; (B) принять предложение целиком, изменив все стандарты, `CONTRIBUTING.md` и валидатор frontmatter; (C) ввести отдельные поля участников и модели, не реабилитируя саму метку. Рекомендация исполнителя — вариант C. | Structured |
...
| ID | Название | Приоритет | Зависимости | Статус | Issue | Источник | Краткое содержание | Режим запуска |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **B-146** | Компиляция четырёх навыков вертикального среза | **P1** | — | todo | [#563](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/563) | [§3 40-practice-and-cases.md](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/blob/main/research/ba-requirements/ba-meta-model/40-practice-and-cases.md) | Выпустить `SKILL.md` вертикального среза `contact-center` по контракту `SK-1`…`SK-6`: шесть обязательных разделов, непустой раздел «Отказ», версионирование только во frontmatter. Режимы запуска в идентичность навыка не входят (ADR-013). **Уточнено версией 0.2 мета-модели (issue [#571](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/571)):** единица компиляции — навык-подпроцесс каталога `L3` процессов `P-01` и `P-03`, а не операция; прежние операции `ingestion`, `understanding`, `structuring`, `documentation` депрекированы ADR-015 как требующие ветвления (`D4`). Состав навыков задаёт `B-161`. | Structured |
| **B-147** | Маршрутный лист среза и схемы контрактов | **P1** | B-146 | todo | [#563](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/563) | [§4 30-decision-framework.md](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/blob/main/research/ba-requirements/ba-meta-model/30-decision-framework.md) | Выпустить маршрут среза с обязательным полем `process` из закрытого словаря (`EP-R1`), актором и гейтами на каждом шаге, и машинные схемы `C-IN`, `C-OUT`. Шаг без навыка помечается человеческим актором (`EP-R2`). **Уточнено версией 0.2 мета-модели (issue [#571](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/571)):** маршрут выпускается двумя документами — граф маршрута и лист прогона (`EP-R5`, `EP-R6`), а процесс `fr-generation` депрекирован ADR-015 (`D1`, `D2`) и заменён на `P-01` `elicitation-processing` и `P-03` `bcreq-formation`. | Structured |
...
## Спринт 17: Пересборка таксономии процессов и операций БА (Мета-модель v2)

**Story.**
Замер на коммитах `8cbf82a` (спица) и `15aa76f` (Хаб) показал восемь дефектов действующей
таксономии: `D1` — 29 мест, где ФТ и ТЗ склеены в один артефакт «ФТ/ТЗ»; `D2` — 4 места, где
граница процесса задана состоянием документа («черновик»), а не классом артефакта; `D3` — 7 из
13 операций без единого промпта; `D4` — 8 из 13 операций требуют ветвления и потому операциями
не являются; `D5` — 6 подпроцессов `П1`…`П6` в спице против 0 упоминаний подпроцесса в Хабе;
`D6` — сущности `Система`, `Пользователь`, `Подпроцесс` отсутствуют в мета-модели; `D7` — 66 из
67 прогонов размечены значениями вне словаря процессов; `D8` — ветвление упомянуто один раз при
двух утверждениях о неизменности маршрута. Корень общий: словарь выведен из исторических
промптов, а не из индустриального базиса, и потому воспроизводит их неточности.

**Цель.**
Перестроить таксономию от индустриального базиса: `L0`/`L1` наследуются из BABOK v3, `L2`/`L3`
объявляются дельтой КК Манго с обоснованием каждого отклонения; операция получает строгий
контракт «один вход — один выходной класс» без ветвления, всё ветвящееся становится навыком;
мета-модель дополняется сущностями `Система` и `Актор` и графовым маршрутом. Правила решения —
...
**Критерий закрытия.**
Задачи спринта закрываются эмпирикой, а не декларацией: базовая линия метрик `MP-*`/`MO-*` снята
на реальных прогонах (`B-157`, `B-159`), а гипотеза переносимости проверена на втором домене
(`B-158`). До этого таксономия остаётся исследованием со `scope: mango-only`, и её перевод в
норму (`B-156`) не запускается.

**Точка останова.** Human Gate перед `B-156`: стандарт мета-модели не выпускается до
подтверждения эмпирикой — иначе будет закреплено непроверенное.

| ID | Название | Приоритет | Зависимости | Статус | Issue | Источник | Краткое содержание | Режим запуска |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **B-155** | Перенос дома мета-модели и таксономий в `projects/<направление>/` | **P2** | — | todo | [#571](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/571) | [§2 50-open-research.md](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/blob/main/research/ba-requirements/ba-meta-model/50-open-research.md) | Признанное расхождение версии 0.2: мета-модель и обе таксономии описывают реализацию БА-процессов КК Манго в GigaCode, то есть являются артефактами конкретного направления, а по правилу проектных артефактов их дом — `projects/<направление>/`, а не общий `research/`. Направление переноса подтверждено владельцем в [комментарии к PR #572](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/pull/572). Перенос затрагивает пути в манифесте, валидаторе структуры и во всех ссылающихся модулях, поэтому вынесен отдельной задачей, а не выполнен попутно. | Structured |
| **B-156** | Стандарт мета-модели БА после первой эмпирики | **P2** | B-155, B-157 | todo | [#571](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/571) | [§5 50-open-research.md](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/blob/main/research/ba-requirements/ba-meta-model/50-open-research.md) | Перевести подтверждённые правила мета-модели (`MM-*`, `SY-*`, `AC-*`, `OPS-*`, `TX-*`) из исследовательского модуля в `standards/` по форме `standard-meta-structure.md`. Запускается только после снятия базовой линии: стандарт до эмпирики закрепит непроверенное. | Structured |
| **B-157** | Базовая линия процессных метрик `MP-1`…`MP-6` | **P1** | — | todo | [#571](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/571) | [§5 20-taxonomy.md процессов](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/blob/main/research/ba-requirements/ba-process-taxonomy/20-taxonomy.md) | Снять `MP-1`…`MP-6` на первых прогонах по новому словарю `P-01`…`P-10` и пересмотреть назначенные ярусы контроля `G1`–`G7`. Ярусы назначены по классу необратимости действия, а не измерены; расхождение ожидается на процессах `P-05` и `P-08`. | Structured |
| **B-158** | Сопоставление `L0` с IREB CPRE и SWEBOK и проверка переносимости | **P2** | B-157 | todo | [#571](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/571) | [ADR-015](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/blob/main/docs/adr/2026-09-adr-015-process-operation-taxonomy-rebuild.md) | Проверить гипотезу переносимости: развернуть таксономию на втором домене без правки `L0`/`L1` и выразить его процессы существующими пятью классами операций. Гипотезу опровергает потребность в новом семействе работ, в операции вне классов или в двенадцатой сущности. BABOK выбран базисом при отвергнутых IREB и SWEBOK — сопоставление с ними закрывает обоснование выбора. | Creative |
| **B-159** | Базовая линия операционных метрик `MO-1`…`MO-5` | **P1** | — | todo | [#571](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/571) | [§5 20-taxonomy.md операций](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/blob/main/research/ba-requirements/ba-operation-taxonomy/20-taxonomy.md) | Снять `MO-1`…`MO-5` на первых прогонах и калибровать ярусы `OG-1`: назначение яруса операции сейчас выведено из необратимости её выхода, а не из наблюдаемой доли отказов гейта. | Structured |
| **B-160** | Эксперимент по воспроизводимости порождающих операций | **P2** | B-159 | todo | [#571](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/571) | [§4 50-open-research.md операций](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/blob/main/research/ba-requirements/ba-operation-taxonomy/50-open-research.md) | Проверить, воспроизводим ли выход операций класса `generate` при одинаковом входе, и если нет — задаёт ли контракт достаточные границы разброса. От результата зависит, применим ли `M-1` к порождающим операциям или для них нужен отдельный измеритель. | Creative |
| **B-161** | Компиляция навыков `L3` в `SKILL.md` исполняющей среды | **P1** | B-146 | todo | [#571](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/571) | [§4 20-taxonomy.md операций](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/blob/main/research/ba-requirements/ba-operation-taxonomy/20-taxonomy.md) | Скомпилировать навыки вертикального среза из разложения «навык → операции»: один `SKILL.md` на **один узел** таксономии — подпроцесс каталога `L3` либо отдельную операцию (`SK-1` в редакции [ADR-016](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/blob/main/docs/adr/2026-09-adr-016-skill-form-and-contract-format.md)). Уточняет `B-146`. | Structured |
| **B-162** | Замер порога объёма навыка относительно окна контекста | **P1** | B-161 | todo | [#571](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/571) | [§3 30-decision-framework.md мета-модели](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/blob/main/research/ba-requirements/ba-meta-model/30-decision-framework.md) | Правило `SK-6` объявляет жёсткую границу объёма навыка — окно контекста среды 1 000 000 токенов, по исчерпании которого прогон не продолжается. Число взято из заявления владельца ([комментарий к PR #572](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/pull/572)), а не измерено. Снять на первых скомпилированных навыках фактический расход: инструкция + входные артефакты + промежуточные результаты, — и назначить рабочий порог разбиения с запасом. До замера `SK-6` опирается на заявление, что объявлено в снимке среды. | Structured |
| **B-163** | Отбор кандидатов в навык-операцию по переиспользованию | **P2** | B-161 | todo | [#571](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/571) | [ADR-016](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/blob/main/docs/adr/2026-09-adr-016-skill-form-and-contract-format.md) | Правило `SB-9` открывает форму «навык на одну операцию», но действующий каталог `L3` не содержит ни одного такого навыка (`F6`). Замер `F7` называет 21 операцию, вызываемую из трёх и более подпроцессов, с максимумом `check-schema` (11 вызовов). Отобрать из них кандидатов на самостоятельную поставку и обосновать отбор переиспользованием, а не удобством. Вынесено отдельной задачей намеренно: документ, вводящий правило, не должен сам же его применять. | Structured |
...
| **B-097** | Автоматизация переноса неактивных PR-с-вопросами в `ops/backlog.md` | **P3** | B-096 | deferred (triggered) | [#475](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/issues/475) | Согласование ОПС с фаундером; RFC [#470](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/pull/470) | Скрипт, который по расписанию или триггеру находит PR с меткой `needs-human-decision` старше N дней, переносит описание вопроса в `backlog.md` и закрывает PR с комментарием. Запуск по запросу; не автоматизировать до явной боли от ручного разбора зависших PR. | Structured |
