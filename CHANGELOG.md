...
- research: датированный замер [Гранулярность навыка и формат контракта](research/ba-requirements/2026-09-10-skill-granularity-format-facts.md) и измеритель [`measure-skill-format.py`](research/ba-requirements/exp/ba-process-taxonomy-571/measure-skill-format.py) (issue #571, [комментарий владельца к PR #572](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/pull/572)) — проверка двух гипотез владельца вместо ответа рассуждением. Документация среды расширена фактами `GC-25`–`GC-28`: навык определён через **повторяемость работы**, а не через уровень декомпозиции, при этом объявлены нижняя граница формы (короткое правило или команда — место в `AGENTS.md`) и верхняя (отдельные права и изолированный контекст — агент). Утверждение версии 0.2 «навык — подпроцесс, а не реализация операции» опоры в документации среды не имеет и было решением Хаба, предъявленным как свойство среды. Замер 24 исторических промптов показал, что имена режимов различали **формат контракта**, а не маршрутизацию: точка возврата управления актору делит корпус без единого пересечения (`stepwise` 10 из 10, `oneshot` 0 из 9, `legacy` 0 из 5), тогда как число шагов не различает ничего — внутренний упорядоченный алгоритм есть у 9 из 9 `oneshot`-промптов, поэтому равенство «`oneshot` = атомарная операция» опровергнуто. В каталоге `L3` навыков из одной операции нет ни одного (2–7 операций на навык), но 21 операция из 31 вызывается тремя и более подпроцессами — отбор кандидатов на самостоятельную поставку вынесен в `B-163`.
- adr: [ADR-016: Навык — форма поставки контракта, формат взаимодействия — вычисляемое свойство](docs/adr/2026-09-adr-016-skill-form-and-contract-format.md) (issue #571, `proposed`) — правила `SF-1`…`SF-7`: навык отделён от уровня декомпозиции и упаковывает **ровно один узел** таксономии — подпроцесс либо одну операцию; границы формы заданы средой (`SF-3`, `SF-4`); формат взаимодействия `interaction` (`dialog` \| `single-pass`) вычисляется по наличию точки возврата управления актору, а не назначается автором. Депрекация имён `stepwise`/`oneshot`/`legacy` как **идентичности** остаётся в силе (ADR-013): ADR-016 не возвращает режимы, а формализует ту ось, которую они смешивали с идентичностью. Уточняет ADR-015, заменяя правило `SB-1`.

- research: датированный снимок [Среда исполнения GigaCode на GitVerse](research/ba-requirements/2026-09-10-gigacode-environment-facts.md) и измеритель [`fetch-gigacode-docs.py`](research/ba-requirements/exp/ba-process-taxonomy-571/fetch-gigacode-docs.py) (issue #571) — одиннадцать страниц документации, названных источником истины в постановке задачи, загружены и проверены: 11 из 11 доступны, 28 контрольных формулировок `GC-1`–`GC-28` найдены. Снимок **опровергает** допущение версии 0.1 мета-модели «детальной документации GigaCode нет, форма `SKILL.md` выведена по аналогии»: контракт навыка задан средой — папка `.agents/skills/<имя>/` с файлом `SKILL.md`, ровно два обязательных поля `name` и `description` (описание пишется как условие применения, потому что по описаниям агент выбирает навык до чтения инструкции), уникальность имени как условие определённости выбора, ступенчатая загрузка. Состав секций Хаба переобъявлен правилом `SK-0` как **дельта поверх контракта среды**, добавлены `SK-8` (описание-условие и уникальность имени) и `GB-5` процессной таксономии (машинный гейт исполним командой оболочки, но не может опираться на `curl` и `wget` — они запрещены профилем прав). Открытый вопрос о механике гейтов закрыт наполовину: `G-mach` исполним внутри прогона, механика `G-human` в документации не описана и остаётся открытой; вместо снятого вопроса «документации нет» записан новый — числовой лимит контекста среды не назван.
- research: [Таксономия процессов БА: индустриальный базис и дельта Манго КК](projects/ba-gigacode-implementation/ba-process-taxonomy/00-introduction.md) и [Таксономия операций БА](projects/ba-gigacode-implementation/ba-operation-taxonomy/00-introduction.md) — два RRP-модуля пересборки таксономии (issue #571, B-155..B-161). Процессы получили четыре уровня: `L0` — шесть областей знаний BABOK v3, `L1` — шесть семейств работ, `L2` — десять процессов КК Манго `P-01`…`P-10`, `L3` — 33 навыка-подпроцесса; `L0`/`L1` наследуются из индустриального стандарта, `L2`/`L3` объявлены дельтой с обоснованием каждого отклонения. Прежние девять процессов депрекированы: граница процесса — смена класса артефакта, а не состояние документа, поэтому `fr-generation` «от сырого запроса до черновика ФТ/ТЗ» распался на отдельные процессы, а ФТ, ТЗ и BCREQ разделены как разные классы выхода. Операция переопределена как атомарное, но семантически завершённое действие с шестиполевым контрактом: 31 операция в пяти классах (`extract`, `transform`, `generate`, `check`, `assess`), всё ветвящееся отнесено к навыкам. Мета-модель обновлена до версии 0.2: одиннадцать сущностей (добавлены `Система` и `Актор`), навык как подпроцесс (`Процесс → навыки 1:N → операции 1:N`), маршрут как направленный граф с условными переходами и прогон как траектория по нему, человеческий гейт с обязательными ссылками на источник (`GT-5`), снятая заявка на универсальность (`scope: mango-only`) и гипотеза переносимости с дорожной картой. Доказательная база — [замер восьми дефектов](research/ba-requirements/2026-09-10-process-taxonomy-defects-facts.md) на коммитах `8cbf82a` и `15aa76f` и [валидатор новой таксономии на семи синтетических кейсах](research/ba-requirements/exp/ba-process-taxonomy-571/README.md).

- adr: [ADR-015: Пересборка таксономии процессов и операций от индустриального базиса](docs/adr/2026-09-adr-015-process-operation-taxonomy-rebuild.md) — правила `TX-1`…`TX-7` (issue #571): таксономия не правится точечно, а пересобирается; `L0`/`L1` наследуются из BABOK v3 без переименования, `L2`/`L3` объявляются дельтой, граница процесса задаётся сменой класса артефакта, у процесса ровно один целевой класс выхода (склейка «ФТ/ТЗ» запрещена), операция атомарна и без ветвления, каждый унаследованный элемент проверяется поимённо, а покрытие таксономии проверяется машинно. Дополняет ADR-014: прежний словарь депрекируется как свидетельство, но не удаляется.
...
- research: [Каноническая мета-модель БА и Execution Package для опытной эксплуатации](projects/ba-gigacode-implementation/ba-meta-model/00-introduction.md) (issue #563, B-146…B-152) — синтез конвейера `M0`–`M4` и двух ортогональных осей в одну исполнимую модель: девять канонических сущностей с ключом идентичности, закон производства `MM-1`…`MM-4`, четыре канонические таксономии (артефакты `A-*`, 13 операций с измеренным покрытием, 9 процессов, продуктовые классы) плюс служебная таксономия гейтов и контрактов, правила компиляции `EP-C1`…`EP-C5`, состав Execution Package из семи классов файлов, контракт скомпилированного `SKILL.md` (`SK-1`…`SK-6`), маршрутный лист (`EP-R1`…`EP-R4`), Golden Set (`EP-G1`…`EP-G5`), Mermaid-модель производства BCREQ со сквозной прослеживаемостью и план вертикального MVP-среза с метриками `M-1`…`M-5` и обязательной базовой линией. Пакет **компилируется, а не портируется**: исследовательские тексты в исполняющую среду не переносятся, а скомпилированный навык самодостаточен — активных ссылок, по которым агент должен ходить во время исполнения, в нём нет.
- adr: [ADR-013: Депрекация режимов запуска `stepwise`, `oneshot`, `legacy`](docs/adr/2026-09-adr-013-run-modes-deprecation.md) (issue #563, `proposed`) — режимы исключаются из активных схем маршрутизации и получают статус исторического свидетельства. Основание измеренное, а не декларативное: 24 промпта реализуют 13 способностей (дублирование 1.85), но варианты одной способности не разделяют структуру (13 пар, средний Жаккар 0.077, ни одной идентичной пары), а маршрутизации по режиму в метаданных прогонов не наблюдается вовсе — 50 из 67 прогонов не ссылаются на промпт. Идентичностью способности становится пара «операция + продуктовый класс». Исторические промпты не удаляются и не переписываются.
- adr: [ADR-014: Историческое наследие — свидетельство, а не базис новой нормы](docs/adr/2026-09-adr-014-legacy-evidence-not-baseline.md) (issue #563, [комментарий фаундера к PR #564](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/pull/564), `proposed`, B-153) — второй контур защиты новой нормы: правила `LG-1`…`LG-6` разделяют роли «свидетельство» и «норма», требуют объявленного происхождения (`derived_from`, `EP-C6`, `SK-7`) и запрещают обосновывать контракт ссылкой «так сделано в промпте или прогоне». Измеренное основание: 10 из 12 нормативных конструкций мета-модели отсутствуют во всех 24 активных промптах.
- standards: [Справочник продуктовой таксономии](standards/product-taxonomy-reference.md) (issue #563, `draft`, B-151) — чистый справочный артефакт Хаба: цепочка наследования Industry (L0) → IT/Telecom (L1) → продуктовая линейка (L2/L3) и иерархия `Domain → Capability → Feature → Atomic Function`, унаследованная от [`product-classification-contract.md`](https://github.com/G-Ivan-A/mango_ba_prompts/blob/main/standards/product-classification-contract.md) спицы, а не изобретённая заново. Операционная информация в справочник не переносится: Хаб владеет формой уровней и правилами отнесения, спица — каталогом значений.
- research: датированный замер [Входные данные мета-модели БА](research/ba-requirements/2026-09-08-meta-model-inputs-facts.md) и evidence-контейнер [`research/ba-requirements/exp/ba-meta-model-563/`](research/ba-requirements/exp/ba-meta-model-563/README.md) (issue #563) — воспроизводимый измеритель идентичности промптов, расхождения скелетов вариантов режима, наблюдаемости маршрутизации, покрытия операций и процессов и продуктовой разметки на коммите `8cbf82a`. Каждое нормативное утверждение мета-модели опирается на измеренный факт, а не на заявление.
- research: датированный замер [Влияние исторического наследия на новую норму](research/ba-requirements/2026-09-09-legacy-normative-influence-facts.md) (issue #563) — проба двенадцати нормативных конструкций по корпусу `8cbf82a` (24 промпта, 67 прогонов) и сплошной аудит происхождения нормативных блоков мета-модели; измеритель [`measure-legacy-normative-influence.py`](research/ba-requirements/exp/ba-meta-model-563/measure-legacy-normative-influence.py).

- research: [Микро-уровень методологии БА: единая структура результирующего артефакта](research/ba-requirements/artifact-micro-structure/00-introduction.md) и [датированный замер вариативности структур](research/ba-requirements/2026-09-08-artifact-structure-variance-facts.md) (issue #561, продолжение каскада #557 вниз, к структуре документа) — работа начата с замера, а не с рамки: [`measure-artifact-structure.py`](research/ba-requirements/exp/ba-micro-structure-561/README.md) обошёл 67 прогонов `runs/` на коммите `8cbf82a`, отделил журналы прогонов и аналитические записки от результирующих документов и дал **17 результирующих документов с 17 различными скелетами** — ни одного совпадения. Расхождение не объясняется разными задачами: внутри одной задачи BCREQ-1074 три артефакта дали средний коэффициент Жаккара 0.69, внутри одного процесса — 0.167–0.333. Устойчива только **арность** разделов, то есть форма нормы «шесть разделов», не несущая смысла. Три факта задали содержание модуля: слот «сценарии» встретился 14 раз под **14 разными подписями** (структура воспроизводится, номенклатура — нет); «UI-логика» и «критерии приёмки» выделены **по одному разу на 17 документов**, чем машинно подтверждается наблюдение фаундера о смешении логики системы и интерфейса; 33 раздела под 27 подписями не отображаются в словарь вовсе, что измеряет долю структуры, определяемой автором, а не нормой. Предложен механизм сведения: слой объявляет **содержание** слота, но не его порядок и подпись (`R-1`), порядок и подписи задаёт Хаб (`R-2`, `R-2.1` — запрет составных подписей «X и Y»), профиль продукта управляет **заполненностью** слота, но не составом скелета (`R-3`), пустой обязательный слот объявляется с причиной из закрытого словаря (`R-4`) — поэтому число слоёв методологии не влияет на число документов у заказчика. Требование «один документ» и требование «разные читатели» разведены проекциями `V-BIZ`/`V-APPROVE`/`V-DEV` одного размеченного ядра с машинно проверяемыми инвариантами `MI-1` (проекция не создаёт содержания) и `MI-2` (объединение проекций равно ядру): три независимых документа дали бы три источника истины. По прямому указанию фаундера **разделены операция и навык** (`OP-1`, `OP-2`): операция — абстрактный тип работы уровня универсальных операций, навык — атомарный скомпилированный артефакт, реализующий ровно одну операцию, процесс — оркестрация навыков, а не навык; отсюда переформулировка `S-R2` — порог двух независимых носителей дефекта применяется к добавлению правила **внутрь** навыка, а не к его заведению, иначе он блокирует покрытие объявленных операций. Самодостаточность навыка получила механизм вместо декларации: `based_on` живёт строго во frontmatter и при нормальном исполнении не читается (`CA-1`…`CA-3`), а связь со слоями удерживается CI-пометкой `OBSOLETE` по версиям, не блокирующей исполнение автоматически (`CA-4`, `CA-5`, задача `B-142`). Golden Set объявлен **обязательным** элементом гейта `G-mach`, а не альтернативой схеме и чек-листу (`GS-1`, `GS-2`); поскольку продуктовая разметка корпуса мультиметочна и чистой выборки по классу не существует, синтез эталонов признан обязательным (`GS-3`) и выполнен: смоделированы [`GS-CC-01`](research/ba-requirements/exp/ba-micro-structure-561/2026-09-08-golden-set-contact-center-ivr-settings.md) для `contact-center` с профилем `P-SETTINGS` и [`GS-LK-01`](research/ba-requirements/exp/ba-micro-structure-561/2026-09-08-golden-set-self-service-lk.md) для `self-service-lk` с профилем `P-NO-SETTINGS` — два продуктовых класса и два класса формата дают **один** скелет, что и проверяет механизм сведения. Эталоны синтетические, имеют статус `draft` и до подтверждения владельцем методологии применяются только структурно (`GS-4`, `GS-5`). Возможность последовательного вызова атомарных навыков проверена **практикой, а не допущением**: субагент прогнал три шага и дал верный результат, но шаг 2 оказался несамодостаточным из-за неоднозначности термина — наблюдение подтвердило инвариант `MI-3` от противного и дало правило `CA-6` (термины навыка берутся из закрытого словаря). Полный цикл человеческого гейта и переносимость на GigaCode остались непроверенными и записаны в раздел «Не выполнено». Сверх примеров фаундера смоделированы 12 граничных кейсов `E-1`…`E-12`, из которых шесть выведены самостоятельно, с указанием уровня предотвращения — методология или промпт. Модуль [`methodology-unification/`](research/ba-requirements/methodology-unification/00-introduction.md) намеренно **не переписан**: поправки к нему оформлены как `П-1`…`П-5` и заведены задачей `B-141`. В [`pr-ops/backlog.md`](pr-ops/backlog.md) добавлен спринт 15 с задачами `B-136`…`B-143` и точкой останова после `B-136`: без утверждённого словаря слотов валидатору нечего проверять, а эталону не с чем сравниваться. Точки фальсификации — `К-M3` (совпадение скелетов внутри задачи равно 1.0) и `К-M5` (нулевая доля разделов вне словаря): если после внедрения словаря скелеты продолжают расходиться, рамка пересматривается, а не расширяется.
- research: [Инвентаризация, карта знаний и двухуровневая рамка методологии БА](research/ba-requirements/methodology-unification/00-introduction.md) и [датированный снимок экосистемы](research/ba-requirements/2026-09-07-ecosystem-knowledge-inventory.md) (issue #557, Шаг 0 каскада) — инвентаризация трёх репозиториев выполнена машинно, а не вручную: [`inventory-knowledge.py`](research/ba-requirements/exp/ba-methodology-unification-557/README.md) обошёл коммиты `c259d61` / `8cbf82a` / `96c288f` и дал 368 артефактов, 3 361 внутреннюю и 200 межрепозиторных связей. Четыре измеренных факта задали содержание работы: **нормативный статус имеют 13 % артефактов** (48 из 368), **БА-релевантен 1 стандарт Хаба из 26** — нормативный слой Хаба посвящён форме документов, а не предмету анализа; **наследование перевёрнуто** — из Манго в Хаб 81 ссылка против 31 обратной, то есть предметное ядро БА живёт в спице со статусом `draft`; **`clarify-engine-ai` не связан с экосистемой ни одной ссылкой**. Отдельно проверено, решает ли задачу картографирования уже существующий артефакт: [`docs/ecosystem-map.md`](docs/ecosystem-map.md) считает единицей репозиторий, а не знание, и не обновлялся три месяца; [RFC конвейера `M0`–`M4`](docs/rfc/2026-08-25-rfc-ba-artifact-pipeline-rrp-roadmap.md) описывает целевое состояние, а не текущее; [`pr-ops/artifact-map.md`](pr-ops/artifact-map.md) однорепозиторный и не содержит графа — вывод: покрытия нет. Предложена **двухуровневая рамка**: управляющий уровень как наследование `L0` (индустрия: BABOK, IEEE 29148, ISO 25010) → `L1` (дельта ИТ-телеком: eTOM, ODA) → `L2` (дельта команды Манго) с правилами `D1`–`D5`, где Манго описывается строго как дельта с обязательным якорем; исполнительный уровень как **разрешённый** самодостаточный артефакт `SKILL.md` (инварианты `I-1`…`I-7`) — дельты скомпилированы заранее, агент исполняет норму, а не вычисляет её во время работы. Слой `L1` в экосистеме отсутствует физически (блокирующий разрыв `G-08`), и это подтверждено эмпирически: разбор [RUN-0064](https://github.com/G-Ivan-A/mango_ba_prompts/blob/main/runs/2026/RUN-0064/reports/error-analysis.md) привлекает области eTOM ad hoc. На прямой вопрос фаундера «субагент прогоняет навыки с самопроверкой или процесс останавливается на человека» дан ответ «постановка вопроса неверна»: гейт распределяется по типу проверки, а не по режиму запуска — `G-self` держит форму, `G-mach` держит всё разрешимое сопоставлением с фактом вне модели, `G-human` держит необратимое. Основание не теоретическое: в [RUN-0018](https://github.com/G-Ivan-A/mango_ba_prompts/blob/main/runs/2026/RUN-0018/metadata.yaml) восемь шагов самопроверки поймали восемь дефектов достоверности и **пропустили один в поставку**, а по корпусу без содержательной правки принимаются 2 прогона из 32 с выставленным вердиктом (6 %). Смоделированы три кейса на реальных прогонах, декомпозиция процесса BCREQ на восемь навыков с тремя уже существующими человеческими гейтами и сквозной проход БА по жизненному циклу `S0`–`S7`. Гипотезы `H-1`…`H-6` снабжены критериями опровержения; каскад из 15 задач `B-121`…`B-135` заведён в [`pr-ops/backlog.md`](pr-ops/backlog.md) тремя этапами с двумя точками останова, где `B-133` — точка фальсификации: если доля прогонов без правки не растёт, каскад останавливается, а не продлевается. По [замечанию фаундера](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/pull/558#issuecomment-5572175813) нулевая связность [`clarify-engine-ai`](https://github.com/G-Ivan-A/clarify-engine-ai) перепроверена по существу: все 53 артефакта прочитаны и размечены экспертно (§5.3 снимка, приложение [`2026-09-07-clarify-relevance-review.md`](research/ba-requirements/exp/ba-methodology-unification-557/2026-09-07-clarify-relevance-review.md)), и вывод переформулирован — **знание есть, канал наследования отсутствует**: 8 механизмов класса `R1` переносимы в методологию (decoding-lock под регрессионным тестом, `Hit Rate@K`/`MRR` на золотом наборе, контракт цитаты, трасса `run_id` на запрос, версионирование промпта, статус-как-машинный-запрет, атомизация требования, замер структуры внешних ТЗ), ещё 9 предметно смежны, 36 вне темы; вне корпуса замера остались `docs/CONCEPT.md` с единственной в экосистеме поимённой привязкой к слою `L0` и работающий `G-self`-судья в `prompts/`. Измеренный ноль рёбер сохранён как факт топологии, но перестал служить основанием содержательного вывода; из этого добавлены `B-134` (подъём механизмов `R1` в норму, закрывает `G-14` содержательно) и `B-135` (правило в корневом `AGENTS.md`: отсутствие связи, статуса или соответствия принятой концепции не является основанием исключить артефакт или вариант из рассмотрения в исследовании Хаба). Утверждения о GigaCode помечены как допущения (вопрос `Q-2`), состав среды «VPS/Serverless» намеренно не определяется и остаётся за Треком Б.
- adr: [ADR-012: Корневой `AGENTS.md` как обязательный артефакт бутстрапа](docs/adr/2026-09-adr-012-agents-md-root-contract.md) (issue #559, B-110/B-111/B-117) — decision record к принятому RFC корневого контракта. Фиксирует шесть решений фаундера: путь легализации — **вариант B**, новый `standards/agents-md-bootstrap-standard.md` (ADR-007 нормирует корень только архетипа A и не переписывается; точечное дополнение оформляется отдельным ADR); **инвариант абсолютных URL** на артефакты Хаба из любого `AGENTS.md`, включая корневой файл самого Хаба; вынос маршрутизации в `ai-rules/agent-work-routing.md` с минимальной таблицей в `AGENTS.md`; наличие-гейт вводится через **grandfathering** на один цикл синхронизации (`structure_grandfather_until`: сначала поименный `WARN`, затем `FAIL`), а не немедленным падением спиц; `AGENTS.md` включается в шаблон задачи как обязательный **SSOT №0**; `CONTRIBUTING.md` и `GOVERNANCE.md` **сохраняются** с разделением ответственности (человеческий workflow / governance-якорь / SSOT агента), вариант удаления отклонён — дублирования функций нет, есть неверное размещение части содержания. Взамен агент-нормативные разделы `CONTRIBUTING.md` переносятся в `ai-rules/` тем же PR, что и соответствующие проверки `require_text`, чем закрывается конфликт двух точек входа `K-1`.


- rfc: [RFC: Ось «Среда», рефакторинг базовых структур и актуализация `AGENTS.md`](docs/rfc/2026-09-04-rfc-bootstrap-environment-and-structure.md) (issue #553, B-117…B-120) — предложение ввести **вторую ось бутстрапа**: среду исполнения (`local` / `gigacode` / `serverless`), ортогональную оси «Архетип». Разделение предметов сформулировано проверяемо: архетип задаёт дома *содержания* (`research/`, `prompts/`, `src/`), среда — *поверхность инструмента* (реестры, конфигурации подключения, точки автозагрузки). Структура репозитория считается по формуле `ядро(ADR-001) ⊕ Δархетип ⊕ Δсреда ⊕ Δпродукт` и семи правилам `R1`–`R7`; ключевое `R1` — **аддитивность**: дельта среды может только добавлять пути и не может отменять дома ядра или архетипа. Аддитивность сводит матрицу `Архетип × Среда` с `N × M` заполняемых клеток к `N + M` независимым правилам и снимает вопрос «какая ось приоритетнее» по построению. Декларация — поля `archetype`, `environment`, `secondary_environments` в уже существующем `.hub-profile.json`, новый файл не заводится (Anti-Inflation); отсутствие поля читается как `local`, поэтому ни один существующий репозиторий не краснеет. При недоопределённом входе исполнитель выводит недостающую ось и фиксирует вывод **отдельным комментарием в PR**: молчаливый дефолт запрещён, неявная среда неотличима от забытой. По рефакторингу базовых структур дана явная позиция исполнителя, как требует контракт задачи. Разделение [`ai-rules/`](ai-rules/README.md) на `commands/` и `skills/` — **согласие с моделью трёх классов** (правило загружается всегда; команда вызывается человеком; навык выбирается агентом по соответствию задаче — различающий тест «кто инициирует применение») при **возражении против создания каталогов в Хабе сейчас**: проверено по факту, что все четыре файла `ai-rules/` (`README.md`, `agent-work-rules.md`, `agent-onboarding-protocol.md`, `adversarial-stress-testing.md`) являются правилами, поэтому разделение дало бы две пустые директории вопреки двухфакторному правилу [ADR-001](docs/adr/2026-06-adr-001-ecosystem-infrastructure-methodology.md) и Anti-Inflation Principle [`pr-ops/repo-model.md`](pr-ops/repo-model.md). Предложено: контракт классов вводится в стандарте, каталоги создаются там, где есть боль, — в спице [`mango_ba_prompts`](https://github.com/G-Ivan-A/mango_ba_prompts). Отдельно разрешено расхождение в самой постановке: гипотеза «структура без `ai-rules/` для среды ГигаКод» противоречит прямому согласованию фаундера в приложенном к issue диалоге («оставить `ai-rules/`, реорганизовать внутреннюю структуру») и отклонена по правилу приоритета диалога и по `R1`. Переименование [`pr-ops/`](pr-ops/README.md) → `ops/` — **согласие с целью при трёх условиях**, причём принято не по аргументу «короче», а по семантике: каталог хранит шесть предметов, из которых операции над PR — один. Цена измерена, а не оценена: 1 087 вхождений в 140 markdown-файлах, 224 строки [`tools/validate-repository-structure.sh`](tools/validate-repository-structure.sh), 158 вхождений в `CHANGELOG.md` и — критично — **247 вхождений в 35 иммутабельных документах `docs/rfc/` и `docs/adr/`, из них 74 markdown-ссылки**. Это создаёт ловушку с двумя одинаково плохими выходами (переписать ссылки → падает гейт [`tools/validate-historical-immutable.sh`](tools/validate-historical-immutable.sh); не переписать → ADR-007 ссылается на несуществующий каталог), поэтому наивная модель «переименовать и переписать ссылки» отклонена. Предложено машинно-узкое исключение из гейта иммутабельности (обратная подстановка обязана дать побайтовое совпадение с base-версией, смысловая правка истории остаётся невозможной), редирект-заглушка на старом пути и один reviewable PR; `CHANGELOG.md` намеренно не переписывается как append-only летопись. Влияние на [RFC корневого контракта `AGENTS.md`](docs/rfc/2026-09-03-rfc-agents-md-root-contract.md) проанализировано, вывод — **правки нужны**, их пять (`A-1`…`A-5`), и `A-1` блокирующая: действующая формулировка `<forbidden>` «модель-специфичные файлы правил запрещены» буквально запрещает нативную поверхность среды GigaCode, то есть корневой контракт запрещал бы Трек А; запрет предложено сузить по существу — «копия правил запрещена, сгенерированный указатель разрешён». План миграции `M0`–`M7c` покрывает Хаб, Mango и Aether-Orbis с правилом отката и без нового механизма льготы. RFC ничего не внедряет: каталоги не переименованы, подкаталоги не созданы, файлы структуры не изменены. Восемь вопросов фаундеру, из них пять блокирующих `Q-1`…`Q-5`. Валидатор структуры дополнен allowlist-записью нового RFC; в [`docs/rfc/README.md`](docs/rfc/README.md) добавлены строки нового RFC и пропущенная строка RFC `AGENTS.md` от 2026-09-03.
- rfc: [RFC: Корневой контракт `AGENTS.md`](docs/rfc/2026-09-03-rfc-agents-md-root-contract.md) обновлён до **v0.2** (issue #551, [комментарий фаундера к PR #552](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/pull/552#issuecomment-5527761347)) — добавлен раздел `P.9` о сосуществовании с корневыми [`CONTRIBUTING.md`](CONTRIBUTING.md) и [`GOVERNANCE.md`](GOVERNANCE.md). Проверено по факту: оба файла обязательны сегодня — они присутствуют в To-Be дереве корня [ADR-007](docs/adr/2026-07-adr-007-hub-root-structure.md) и в массиве `required_files` [`tools/validate-repository-structure.sh`](tools/validate-repository-structure.sh), причём на `CONTRIBUTING.md` навешено около тридцати проверок `require_text`; `pr-ops/repo-model.md` набор корневых файлов не нормирует (его таблица описывает только каталоги). Конфликты: `K-1` — реальный конфликт маршрутизации, раздел «AI-Assisted Work» в `CONTRIBUTING.md` объявляет точкой входа агента `GOVERNANCE.md`, что противоречит модели «`AGENTS.md` — SSOT №0»; `K-2`/`K-3` — риск раздвоения SSOT: агент-нормативные разделы («Правило авто-заполнения Мета», «Специфика работы с AI-агентами», «Работа с внешними источниками», запрет метки `no-diff-expected`) живут вне `ai-rules/`, и тот же дефект воспроизведён в `templates/htom/`; `K-4`/`K-5` — конфликта нет: `GOVERNANCE.md` уже тонкий якорь на 23 строки без нормативного текста, а тематические пересечения с `CONTRIBUTING.md` ссылаются на одни и те же стандарты. Удаление обоих файлов (вариант `D`) **отклонено** с обоснованием: оба входят в `required_files`, поэтому удаление ослабило бы жёсткий слой, который RFC усиливает; `CONTRIBUTING.md` — конвенция GitHub для человека, `AGENTS.md` — конвенция агентных инструментов, аудитории и поверхности разные; стоимость несоразмерна scope (ADR-007, валидатор, `templates/htom/`, `templates/spoke/` и пять стандартов, включая построенный вокруг `CONTRIBUTING.md` [`standards/team-contract.md`](standards/team-contract.md)). Вместо удаления предложено разделение ответственности и инвариант «ни один корневой файл, кроме `AGENTS.md`, не содержит нормативных правил для агента», машинно проверяемый двумя новыми условиями гейта `P.7`. Добавлен блокирующий вопрос `Q-6`.
- rfc: [RFC: Корневой контракт `AGENTS.md`](docs/rfc/2026-09-03-rfc-agents-md-root-contract.md) (issue #551, B-110) — предложение легализовать корневой `AGENTS.md` как обязательный артефакт бутстрапа для всех архетипов (A/B/C/D). Мягкий слой (диспетчер: онбординг, маршрутизация в `ai-rules/`, эскалация) + жёсткий слой (валидатор падает без `AGENTS.md`). Три варианта пути легализации (рекомендован новый стандарт бутстрапа), структура `AGENTS.md`/`agent-work-routing.md`/`agent-work-rules.md` на базе черновика, граф приоритетов «контракт → исполнение / эскалация», правило об отсутствующих тегах (эскалация без остановки), адаптация под архетипы, инвариант абсолютных ссылок и пять блокирующих вопросов фаундеру Q-1…Q-5. RFC ничего не внедряет — дополняет B-110…B-116, не отменяя. Валидатор структуры дополнен allowlist-записью нового RFC.
- analysis: [Анализ коренных причин несоблюдения правил экосистемы ИИ-агентами](docs/analysis/2026-09-03-ai-rules-compliance-failure-root-cause.md) (issue #547) — прямо зафиксировано, что текущая архитектура правил провалилась в обеспечении compliance со стороны ИИ. Пять проверенных причин: отсутствие корневой точки входа, пробел в стандарте контрактной документации (формат задан, дом не задан — отсюда `docs/contracts/` в Mango), отсутствие доставки генома в Aether-Orbis (каталога `.github/ISSUE_TEMPLATE` там нет), смешение уровней постановки в самом шаблоне Хаба, раздутие правил как усилитель. Обоснована недостаточность одного `AGENTS.md` и обязательность машинных гейтов.
- research: [Индустриальные практики онбординга ИИ-агентов](research/hub/2026-09-03-ai-agent-onboarding-entrypoint-practices.md) (issue #547) — разбор AGENTS.md, CLAUDE.md, `.github/copilot-instructions.md`, `.cursor/rules/` и эффекта «lost in the middle»; сводная матрица механизмов доставки и шесть выводов В1–В6 для экосистемы.
- templates: [Черновик корневого `AGENTS.md`](templates/agents-md-root-draft.md) (issue #547) — единый диспетчер правил для всех моделей: XML-секционирование для парсинга LLM, явные запреты (включая `docs/contracts/`), таблица маршрутизации абсолютными URL, дома артефактов, пять уровней постановки и таблица автозагрузки по инструментам. Размещение в корне — задача B-110.
...
- ops: дом мета-модели БА и таксономий процессов и операций перенесён из `research/ba-requirements/` в [`projects/ba-gigacode-implementation/`](projects/ba-gigacode-implementation/README.md) (issue #573, B-155, направление подтверждено владельцем в [комментарии к PR #572](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/pull/572)) — три RRP-модуля ([мета-модель](projects/ba-gigacode-implementation/ba-meta-model/00-introduction.md), [таксономия процессов](projects/ba-gigacode-implementation/ba-process-taxonomy/00-introduction.md), [таксономия операций](projects/ba-gigacode-implementation/ba-operation-taxonomy/00-introduction.md)) описывают реализацию БА-процессов КК Манго в GigaCode, то есть являются артефактами конкретного направления, а их дом по правилу проектных артефактов — `projects/<направление>/`, а не общий `research/`. Перемещение выполнено `git mv` с сохранением истории; все ссылающиеся пути обновлены сплошным переписыванием по репозиторию, миграция объявлена в `path_migrations` профиля Хаба. Датированные снимки фактов и экспериментальные измерители сознательно остались в `research/ba-requirements/`: это доказательная база, а не артефакт направления. Валидатор структуры получил новые пути в whitelist и в `required_files`, а [`tools/validate-rrp-links.sh`](tools/validate-rrp-links.sh) — второй корень поиска `projects`, иначе проверка правила P2 молча перестала бы покрывать перенесённые модули.

- research: правила уровней декомпозиции `SB-*` в [теории таксономии процессов](projects/ba-gigacode-implementation/ba-process-taxonomy/10-theory.md) переформулированы по ADR-016 — `SB-1`…`SB-5` говорят о **подпроцессе**, `SB-6`…`SB-9` вводят навык как форму поставки, добавлены правила формата `FM-1`…`FM-5`. Контракт `SKILL.md` мета-модели дополнен: `SK-1` допускает навык на одну операцию, `SK-6` получил **жёсткую границу объёма** — окно контекста среды 1 000 000 токенов, по исчерпании которого прогон не продолжается (заявление владельца; замер вынесен в `B-162`), добавлены `SK-9` (обязательное вычисляемое поле `interaction`) и `SK-10` (границы формы), реестр депрекации — правило `DP-5`. Контракт человеческого гейта [таксономии процессов](projects/ba-gigacode-implementation/ba-process-taxonomy/30-decision-framework.md) дополнен правилами `HGC-5`…`HGC-7`: технический мерж или approve pull request **не является** прохождением `G-human` — это процедурный шаг, тогда как содержательный гейт требует явного вердикта ревьюера с перечнем фактически прочитанных документов, решением, основанием и подписью; отсутствие вердикта при наличии мержа — дефект, а не основание считать гейт пройденным.

- research: [Микро-уровень методологии БА](research/ba-requirements/artifact-micro-structure/00-introduction.md) доработан по [комментарию фаундера к PR #562](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/pull/562) — закрыты четыре названных разрыва. **Покрытие эталонов доведено до всех четырёх профилей** (правило `GS-6`): к `GS-CC-01` и `GS-LK-01` добавлены `GS-API-01` (`voice-ucaas`), `GS-DEV-01` (`hardware`), `GS-SEC-01` (`security`) и `GS-MP-01` (два продукта в одной задаче); для доменов `hardware` и `security` документов нет ни в `runs/`, ни в `kb/processed`, поэтому эталоны синтезированы из правил профиля, а недостающие продуктовые решения выражены **блокирующими вопросами**, а не правдоподобными числами — в `GS-DEV-01` их три из трёх. **Разрешён конфликт профилей при нескольких продуктах** (`MP-1`…`MP-6`, §7 [`20-taxonomy.md`](research/ba-requirements/artifact-micro-structure/20-taxonomy.md)): профиль задаёт не запрет слота, а значение заполненности по умолчанию, поэтому объединение всегда определено и арбитр не нужен — слот заполнен, если заполнен хотя бы для одного продукта, а часть отсутствующего продукта объявляется причиной `нет по продукту`; на прямой вопрос «продукт А `P-DEVICE`, продукт Б `P-SETTINGS` — нужен ли `S-UI`» ответ «да, и он присутствовал бы даже при обоих пустых профилях», иначе возвращается неотличимость «нет по продукту» от «забыли». **Словарь слотов проверен на независимом корпусе** [`kb/processed`](research/ba-requirements/2026-09-08-kb-slot-fit-facts.md), из которого он не выводился: 5 документов, 860 разделов, **0 разделов потребовали слота вне словаря**, все 5 предзарегистрированных порядковых утверждений о профилях подтвердились на всех парах документов; два пороговых предсказания в абсолютных долях **не подтвердились** и оставлены в инструменте и отчёте — из них следует, что профили по умолчанию для `analytics` и `ai-automation` не подтверждены (`B-143`). **Единый скелет BCREQ выделен в отдельный артефакт** [`2026-09-08-bcreq-document-skeleton.md`](research/ba-requirements/exp/ba-micro-structure-561/2026-09-08-bcreq-document-skeleton.md): управляющая таблица «слот × обязательность × профиль × проекция × класс формата» на 14 слотов, закрытый словарь причин пустоты и сам скелет; `GS-CC-01` и `GS-LK-01` приведены к нему, чем устранена их составная подпись «Ограничения и открытые вопросы», нарушавшая собственное правило модуля `R-2.1`. Добавлены граничные кейсы `E-13`…`E-16`, вопросы `QM-6`, `QM-7` и задачи бэклога `B-144`, `B-145`.
- rfc: [RFC: Корневой контракт `AGENTS.md`](docs/rfc/2026-09-03-rfc-agents-md-root-contract.md) переведён в `accepted` (`v0.3` → `v1.0`, issue #559): заполнены `Decision record` (ADR-012) и `Implementation link` (B-110), в Open Questions внесены ответы фаундера по `Q-1`…`Q-6`. Внесены четыре доопределяющие правки. **`A-6`: объём нормируется в токенах, а не в строках** — мягкий порог 4K токенов (рекомендация выделить объёмный блок в суб-контракт со ссылкой) и жёсткий порог 8K токенов (`FAIL`); основание — U-образная деградация использования длинного контекста ([Liu et al., «Lost in the Middle», TACL 2023](https://arxiv.org/abs/2307.03172)), из-за которой обязательные запреты в середине разросшегося файла читаются хуже всего. Описан механизм выделения: блок переносится в каноничный дом целиком, на его месте остаётся одна строка с абсолютным URL без копии правила (`R4`), `<hard_rules>` и `<forbidden>` выносу не подлежат, выделение выполняется тем же PR, висячая ссылка = `FAIL`. **`A-7`**: навык (Skill) запускается и человеком через `/<slug>`, и агентом по контексту задачи; явный вызов человеком не переводит навык в класс «команда» — класс задаётся формой артефакта. **`A-8`: гейт ≠ валидатор** — жёсткий слой разделён на машинные гейты (детерминированный `PASS`/`FAIL`: валидаторы, pre-commit, CI) и контрактные гейты (обязательства уровня промпта, проверяемые самопроверкой агента и human review); норма, машинно выразимая, обязана стать машинным гейтом. **`A-9`, правило `R8` «декларация вместо запрета»**: репозиторий может содержать каталоги вне канонического набора при декларации в `.hub-profile.json` с обязательным непустым `reason`; отсутствие каталога в каноническом наборе не равно запрету.
- rfc: [RFC: Ось «Среда», рефакторинг базовых структур и актуализация `AGENTS.md`](docs/rfc/2026-09-04-rfc-bootstrap-environment-and-structure.md) (`v0.2` → `v0.3`) — раздел `P.9.2` и правило приёмки `V-10` пересмотрены по решению фаундера в issue #559: **машинный denylist деклараций для имён `plans/` и `tasks/` отменён**. Поимённые denylist-ы каталогов запрещены; легализация выполняется общим механизмом `R8`. Содержательное требование «планы и задачи не образуют второго SSOT состояния работ» сохранено, но переведено в контрактный гейт: машинно проверяется только наличие непустого `reason`, а его осмысленность — при human review. Правка допустима без нового ADR, поскольку RFC находится в статусе `proposed`, то есть до decision gate.
- template: [`templates/agents-md-root-draft.md`](templates/agents-md-root-draft.md) (`v0.2` → `v0.3`) — строка `<forbidden>` о каталогах планирования заменена запретом на поимённые denylist-ы каталогов с описанием механизма декларации (`R8`); в `<scope>` добавлены пороги объёма 4K/8K токенов (`A-6`); в `<artifact_homes>` уточнено, что навык может быть вызван и человеком через `/<slug>` (`A-7`).
- ops: [`pr-ops/backlog.md`](pr-ops/backlog.md) (`1.56` → `1.57`) — B-110, B-111 и B-117 ссылаются на ADR-012 как на decision record; в B-110 зафиксирован принятый путь легализации (вариант B), в B-117 запрет объявления `plans/`/`tasks/` заменён правилом `R8`, в B-120 переформулировано правило `V-10`.
- rfc: решения фаундера по восьми вопросам [RFC оси «Среда»](docs/rfc/2026-09-04-rfc-bootstrap-environment-and-structure.md) зафиксированы в тексте документа, RFC переведён `v0.1 draft` → **`v0.2 proposed`** (issue #555). Раздел «Решения фаундера» в `Open Questions` даёт по каждому вопросу `Q-1`…`Q-8` статус, решение и следствие; исходные формулировки сохранены как история постановки. Приняты: ось «Среда» вместе с правилом аддитивности `R1` (`Q-1`), сохранение `ai-rules/` с внутренней реорганизацией вместо структуры без него (`Q-3`), три условия переименования `pr-ops/` → `ops/` (`Q-4`). Новый раздел `P.9` доопределяет три нормы для стандарта бутстрапа. `P.9.1` (`Q-6`): базовый шаблон бутстрапа создаёт `ai-rules/commands/` и `ai-rules/skills/` с заглушкой `README.md` (`status: placeholder`) при создании репозитория под целевую среду — с явной границей, снимающей противоречие с порогом «не менее двух артефактов» из `P.5`: заглушка допустима только в момент бутстрапа как часть поставляемого генома, создание пустого каталога отдельным PR в существующем репозитории запрещено. `P.9.2` (`Q-7`): отдельное правило-запрет на `plans/`/`tasks/` **не вводится** — проверено, что валидатор генома [`templates/htom/tools/validate-repository-structure.sh`](templates/htom/tools/validate-repository-structure.sh) обходит `find . -maxdepth 1 -type d` и уже даёт `FAIL` на любом недекларированном корневом каталоге, поэтому второе выражение той же нормы породило бы вопрос «какое из двух SSOT»; вводится минимальная норма, закрывающая единственную реальную дыру, — запрет объявлять эти имена через `project_specific_directories`. `P.9.3` (`Q-8`а): термин `serverless` называет модель тарификации вычислений, а не предмет оси, и ложно сужает (VPS сервером управляет); предложены три кандидата с разбором за/против — `hosted-agent` (рекомендация исполнителя), `vps-runtime`, `standalone-agent`; состав дельты среды отложен до RFC Трека Б (`Q-8`б). Добавлены правила приёмки `V-10` и `V-11`.

- rfc: [RFC корневого контракта `AGENTS.md`](docs/rfc/2026-09-03-rfc-agents-md-root-contract.md) `v0.2` → **`v0.3`** и черновик [`templates/agents-md-root-draft.md`](templates/agents-md-root-draft.md) `v0.1` → **`v0.2`** — внесены правки `A-1`…`A-5` из раздела `P.7` RFC оси «Среда» по решению фаундера `Q-5` (issue #555): правки вносит исполнитель, а не владелец документа отдельным циклом. `A-1` (блокирующая, снимает блокировку Трека А): запрет переформулирован по существу, а не по имени файла — **копия** правил в модель- или среда-специфичном файле запрещена, **сгенерированный указатель** на каноничный артефакт разрешён при условии, что файл помечен как сгенерированный и его ручная правка падает в валидаторе; предмет запрета (второй SSOT) не ослаблен. `A-2`: в `<scope>` добавлены обязательные строки `archetype` и `environment` со значениями из `.hub-profile.json`, SSOT — профиль, расхождение = `FAIL`. `A-3`: запрет на новые top-level каталоги уточнён оговоркой «не предусмотренные ядром, архетипом, объявленной средой или декларацией `project_specific_directories`». `A-4`: раздел «Архетипы репозиториев» → «Архетипы и среды», дельта спицы записана как `Δархетип ⊕ Δсреда`. `A-5`: (а) наличие-гейт обязан сначала прочитать `environment` из профиля и лишь затем проверять требуемый набор; (б) в маршрутизацию добавлены `ai-rules/commands/` и `ai-rules/skills/`, а переписывание `pr-ops/*` → `ops/*` явно привязано к PR миграции (B-119), чтобы маршрутизатор не указывал в несуществующий путь до неё.

- backlog: [pr-ops/backlog.md](pr-ops/backlog.md) v1.55 → **v1.56** (issue #555) — зарегистрированы задачи по итогам решений фаундера: **B-121** (исследование фактического механизма discovery навыков в GigaCode по `Q-2`; до результата действует `R4`, поэтому Трек А не блокируется), **B-122** (отложенный RFC по замене `ops/backlog.md` внешним трекером через MCP по `Q-7`, триггер — боль ручного ведения бэклога), **B-123** (отложенный состав дельты среды комплексного развёртывания и утверждение её имени по `Q-8`б, триггер — принятие RFC Трека Б). Сняты пометки «требует решения фаундера» с B-117 (`Q-1`…`Q-3`) и B-119 (`Q-4`); B-117 дополнена нормами `P.9.1` и `P.9.2`, B-120 — проверкой `V-10` и отметкой, что правки `A-1`…`A-5` уже внесены в RFC и черновик.
...
- standards: [issue-workflow.md](standards/issue-workflow.md) v1.3 → **v1.4** (issue #547) — раздел «Связь с `User Story / ФТ`» заменён на «Пять уровней постановки задачи» с таблицей «уровень → содержание → субъект» и правилом невозможности блокировать исполнение из-за неполноты постановки.
- backlog: добавлен [Спринт 13](pr-ops/backlog.md) «Принудительный онбординг ИИ-агентов — корневой AGENTS.md и уровни постановки» (issue #547) — задачи B-110…B-116: легализация `AGENTS.md`, скрипт `tools/inject-agents-md.sh`, исправление структуры Mango и Aether-Orbis, стандарт правил Story/ФТ/НФТ, легитимизация шаблонов и машинные гейты соблюдения правил.
