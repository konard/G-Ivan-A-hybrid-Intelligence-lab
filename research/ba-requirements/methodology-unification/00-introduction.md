...
4. **У `clarify-engine-ai` знание есть, а канала наследования нет.** Ссылок с
   Хабом и Mango нет ни в одну сторону (1 входящее ребро, 0 исходящих, 53
   артефакта без frontmatter), но сплошное прочтение всех 53 артефактов даёт
   **8 механизмов, переносимых в методологию** (атомизация требования,
   decoding-lock, метрики приёмки, контракт цитаты, трасса на запрос,
   версионирование промпта, статус-как-гейт) и ещё 9 предметно смежных; вне
   корпуса остался
   [`docs/CONCEPT.md`](https://github.com/G-Ivan-A/clarify-engine-ai/blob/main/docs/CONCEPT.md)
   — единственный в экосистеме документ с поимённой привязкой к слою `L0`.
   Разметка: §5.3
   [замера](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/blob/main/research/ba-requirements/2026-09-07-ecosystem-knowledge-inventory.md).
