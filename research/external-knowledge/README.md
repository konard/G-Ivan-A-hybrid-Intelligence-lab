...
| --- | --- | --- |
| Источник изучен (всегда, независимо от пользы) | Запись в [реестр](external-sources-registry.md) — **ВСЕГДА** | — |
| Из источника извлечён полезный вывод | Инсайт-файл в [external-insights/](external-insights/) + ссылка из реестра | — |
| Источник бесполезен / нерелевантен | Только реестр со `stage`-статусом и пометкой «бесполезен» | **НЕ** создаём инсайт в `external-insights/` |

> ⛔ **Anti-Inflation.** В базу знаний (`external-insights/`) попадает только
> польза. Бесполезный источник остаётся **фактом в реестре** и не превращается в
> отдельный файл-инсайт.
