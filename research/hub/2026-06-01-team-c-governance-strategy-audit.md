...
`AI_GOVERNANCE.md`, `CONTRIBUTING.md`, `ops/repo-model.md`,
`standards/README.md`, `standards/research-profile.md`,
`standards/issue-workflow.md` и `ops/artifact-map.md`.
...
| --- | --- | --- |
| Concept | Зачем существует репозиторий и где его границы. | `docs/concept.md` |
| Governance contracts | Как работают люди и ИИ, кто принимает решения. | `AI_GOVERNANCE.md`, `CONTRIBUTING.md` |
| Repository model | Где живут артефакты и когда создаются новые. | `ops/repo-model.md` |
...
| --- | --- | --- | --- |
| `TC-GOV-001` | Закрепить принцип "нет governance-артефакта без операционной боли". | Снижает риск governance overgrowth. | Создать issue на уточнение `ops/repo-model.md` после review этого research. |
...
2. Нужна ли отдельная governance hierarchy сейчас, или достаточно встроить
   таблицу hierarchy в существующий `ops/repo-model.md`?
...
| `ops/repo-model.md` | Anti-Inflation principle и правило размещения артефактов. |
