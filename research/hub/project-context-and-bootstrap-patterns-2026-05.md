...
| --- | --- | --- |
| Project context живет рядом с проектом, а не в общем промпте. | `archive/projects/mango/README.md` описывает проект, стандарты, prompt assets и связь с `research/mango/`. | README проекта подходит как первый источник контекста для нового чата. |
| Research и project artifacts разделены по ответственности. | `research/mango/` хранит классификацию, lifecycle и RAG-roadmap; `archive/projects/mango/` хранит prompts, experiments и Mango-only glossary. | Новому проекту не нужно копировать все research-артефакты внутрь себя. |
...
| `_exp` и `_simple` варианты решают проблему доступности контекста. | `archive/projects/mango/prompts/*_exp-2026-05.md` ссылаются на repo context, а `*_simple-2026-05.md` работают через вопросы. | Если контекст недоступен, ИИ должен перейти к диалогу, а не выдумывать missing knowledge. |
...
| --- | --- | --- |
| Быстрый контекст не выделен отдельным блоком. | `archive/projects/mango/README.md` богатый, но новому чату нужно вручную выжимать цель, статус и следующий шаг. | При смене чата растут затраты токенов и риск потерять актуальное состояние. |
...
| Placeholder-папки выглядят как обязательный шаблон. | `archive/projects/mango/README.md` показывает `kb/`, `docs/`, `experiments/`, `decisions/` как точки расширения. | Новый проект может скопировать больше папок, чем реально нужно в первые часы. |
...
| --- | --- | --- | --- |
| 1 | Потеря контекста при смене чата. | Контекст проекта распределен между `archive/projects/mango/README.md`, `research/mango/`, prompt files и experiments. | Новый ИИ получает неполную историю или тратит время на повторное чтение всего дерева. |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `REC-001` | `archive/projects/mango/experiments/prompts-selftest-2026-05-26.md` | Добавить единый prompt с режимом `exp | simple`. | mango-only | medium | small | idea | обсудить с PO | - |
