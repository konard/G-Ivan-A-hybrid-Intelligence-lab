...
([governance/proposals/rfc-agent-onboarding-protocol.md](../../governance/proposals/rfc-agent-onboarding-protocol.md))
внешняя экспертная группа (далее — **Команда С**) выполнила архитектурный review
RFC через призму emerging governance-паттернов в agent ecosystems. Цель этого
файла — зафиксировать рекомендации Команды С как **исследовательский артефакт**
(`Artifact`, см. глоссарий) и предложить матрицу применимости, которая отвечает
на один вопрос: *что брать сейчас, что отложить и что отклонить*, не перегружая
текущую реализацию.
...
([governance/REPO_MODEL.md](../../governance/REPO_MODEL.md)) и вывода аудита
...
> источник» из
> [rfc-agent-onboarding-protocol.md](../../governance/proposals/rfc-agent-onboarding-protocol.md).
...
| Threat awareness | Раздел «Что может пойти не так»: 3–5 рисков в человекочитаемом виде. | `governance/AGENT_ONBOARDING.md` (проектируется в RFC онбординга) | `[C5]` |
| Evidence model (назвать явно) | Зафиксировать тезис: «git history + issues + PRs = evidence trail». Функция уже есть — нужно дать ей имя. | RFC-манифест `governance/proposals/rfc-two-cases-of-project-initialization.md` | `[C5]`, `[GAP]` |
| Approval semantics (простые) | Поле `approved_by:` во frontmatter критичных файлов как явный след апрува. | `standards/FILE_NAMING.md` | `[C5]`, `[EGA]` |
...
- Документ **не создаёт** файлы улучшений (`AI_QUICK_RULES.md`,
  `AI_GOVERNANCE.md`, `AGENT_ONBOARDING.md` и т.д.). Колонка «Куда встроить» в
...
| --- | --- |
| [governance/proposals/rfc-agent-onboarding-protocol.md](../../governance/proposals/rfc-agent-onboarding-protocol.md) | RFC онбординга (Кейс 1), ради review которого Команда С дала рекомендации. |
| [governance/proposals/rfc-two-cases-of-project-initialization.md](../../governance/proposals/rfc-two-cases-of-project-initialization.md) | RFC-манифест двух кейсов инициализации (в работе); целевое место для «Evidence model — назвать явно». |
