...
([`repository-structure-concept-2026-06.md`](repository-structure-concept-2026-06.md))
...
**Связанные артефакты.**
- [`research/hub/repository-structure-concept-2026-06.md`](repository-structure-concept-2026-06.md)
...
(b) прямо продолжает [`repository-structure-concept-2026-06.md`](repository-structure-concept-2026-06.md),
...
| **(δ) Продуктовая таксономия** | Domain | Capability | Feature | Atomic Function | mango `classification.md` v3.0 |
...
> **Кросс-репозиторное замечание:** файлы `research/mango/{classification, classification-tz,
> requirements-flow, taxonomy-concept-2026-05}.md` **отсутствуют** в текущем mango
> (проверено: `find` не нашёл) — они **мигрированы** из спока в Хаб (frontmatter
> `classification.md`: «source: git history before migration»). Это **не дубль**,
...
### 9.1. Концепция структуры каталогов (`repository-structure-concept-2026-06.md`)
...
- [research/hub/ecosystem-architecture-research-2026-06.md](ecosystem-architecture-research-2026-06.md)
...
- [research/mango/classification.md](../mango/classification.md) — таксономия δ
...
Замер именования в репозитории (для информирования выбора, не для адаптации к нему):

| Паттерн | Пример | Встречаемость (замер) |
| --- | --- | --- |
| **Суффикс-дата** `name-YYYY-MM` | `artifact-inventory-and-classification-2026-06.md`, `migration-audit-2026-05.md` | ~31 файл (доминирующий) |
| **Префикс-дата** `YYYY-MM-name` | (часть заметок) | ~7 файлов |
| **Без даты** | `classification-glossary.md` | ~14 файлов |

`standards/file-naming.md` (v1.4) фиксирует `lowercase-with-hyphens` и ISO 8601
`YYYY-MM-DD` для **полей** датировки, и приводит пример `migration-audit-2026-05.md`
(дата-**суффикс**), но **не предписывает** префикс-vs-суффикс именно в имени файла.
Здесь — реальная развилка стандарта.
...
- *Trade-off:* файлы сортируются по времени; **расходится** с доминирующим в репо
  суффиксом (~31 файл), потребует миграции/двойного стандарта.

**Вариант B — дата-суффикс `name-YYYY-MM.md`.**

- *Практика:* доминирующая конвенция самого репо; пример в `file-naming.md`
  (`migration-audit-2026-05.md`); группировка по теме при сортировке.
- *Trade-off:* семантика темы видна первой, согласуется с текущим; **хуже**
...
| Вариант | Сортировка по времени | Согласие с текущим репо | Стабильность ссылки | Практика |
| --- | --- | --- | --- | --- |
| A префикс | ✅ | ❌ (миграция) | ⚠️ | Hugo/Jekyll/Zettelkasten |
| B суффикс | ❌ | ✅ доминирует | ⚠️ | репо + `file-naming.md` |
| C полная дата | ✅ | ❌ | ⚠️ | Hugo/Jekyll |
| D номер | по созданию | ❌ | ✅ | ADR/MADR/IETF |
| E semantic | ❌ (дата в FM) | ⚠️ частично | ✅ | docs-as-code/wiki |

Выбор стандарта именования — **решение фаундера**; при выборе A/C/D потребуется
явное решение о миграции существующих ~31 файла (или о двойном стандарте «новые vs
унаследованные», как уже допускает `file-naming.md`).
