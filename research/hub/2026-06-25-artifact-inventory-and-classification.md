...
| `projects-sink/AI_PROJECT_CONTEXT-Summary.md` | навигация | L1 | governance | 0.7K | Сжатый контекст проекта. active |
| `docs/concept.md` | концепция | L1 | governance | 1.7K | Цель/границы Хаба. active |
...
| **L1** | **Архетип / Домен** (high-level) | Высокоуровневые сущности: архетипы, домены, видения, концепции, карты экосистемы | «Отвечает на вопрос *что за мир/проект*, без деталей реализации» | архетипы PR #243, `docs/concept.md`, `vision.md`, `ecosystem-map.md`, `README.md`-навигация, Product Vision (β-L1), Domain (δ) |
...
| 5 | RFC mango | `docs/analysis/` **и** `docs/rfc/` | §2 (Q2): RFC — деление по функции | ⚠️ Двойственность | Свести RFC в один канон (`docs/rfc/`); `docs/analysis/` — только анализ |
...
`docs/analysis/` и `docs/rfc/`, а критерий процессных промптов (§7.1
...
| RFC ↔ ADR | Хаб: только RFC (`docs/rfc/`), 0 ADR; mango: RFC + 13 ADR (`docs/adr/`) + 2 в `standards/decisions/` | ⚠️ ADR-локация раздвоена (§8.2) |
...
RFC — в **двух** (`docs/analysis/` и `docs/rfc/`). Это ровно «дублирование
...
| Маршрут: контракт→governance, RFC→docs/analysis | ⚠️ RFC и в `docs/rfc/` тоже | двойственность (§8.5) |
...
| 6 | mango `docs/analysis/` смешивает анализ + RFC + convergence-тесты (21 файл) | **Изоляция/смешение** типов | — | **Не удалять**; рассортировать (RFC→`docs/rfc/`) |
...
3. Это ПОВТОРЯЕМОЕ правило/категория для класса артефактов?
   ...
   3b. Решение «что менять» (proposal)? → L2/governance→ docs/rfc/
...
| RT-04 | RFC — в `docs/rfc/`; `docs/analysis/` — только `type: analysis|research` | детект `type: rfc` в docs/analysis → warn |
...
| Анализ оформлен как предложение изменения | `docs/analysis/` → `docs/rfc/` |
| RFC принят как решение | `docs/rfc/` → `docs/adr/` (record) |
...
| RFC-локация mango | `docs/analysis/` | `docs/rfc/` | **`docs/rfc/`** канон; analysis — только анализ |
...
5. **Свести ли RFC mango в `docs/rfc/`** (разгрузить `docs/analysis/`)?
