...
[research/external-knowledge/2026-06-wigers-requirements-analysis.md](../../research/external-knowledge/2026-06-wigers-requirements-analysis.md))
...
| **Инжиниринг промптов / prompt engineering** как **подпроцесс** разработки AI-решения (НЕ тип требования) | Классификация AI-разработки ниже: Prompt = интерфейс к Tool, инжиниринг промптов = подпроцесс, Prompt specification = артефакт. | E1–E2 (Candidate) |
...
## Классификация AI-разработки: исправление (prompt engineering ≠ тип требования)

> 🔧 **Контекст исправления.** В ранней редакции (PR #248) инжиниринг промптов был
> ошибочно отнесён к **типам требований**. Это ошибка классификации: тип требования
> отвечает на вопрос **ЧТО** требуется (Функция / Качество / Ограничение /
> Бизнес-правило), а инжиниринг промптов — это **КАК** мы это достигаем
> (подпроцесс разработки). Ниже — исправленная модель.

### Prompt = интерфейс к Tool (а не сам Tool)

В модели сотрудничества Хаба **Human → Agent → Tool → Knowledge** промпт занимает
место **интерфейса к Tool**, а не отдельного Tool и тем более не типа требования:

- **Tool** — это LLM (например, GPT-4o, Claude) или иной исполнитель.
- **Prompt** — это **интерфейс** к этому Tool: текст-обращение, через который Agent
  передаёт Tool задачу и контекст.
- **Аналогия (двуязычно).** SQL-запрос (`SQL query`) ≠ база данных (`database`);
  SQL-запрос — это **интерфейс** к базе данных. Точно так же `prompt` ≠ `LLM`;
  `prompt` — интерфейс к LLM. Промпт не «новый тип требования», а способ обращения
  к инструменту.

### Инжиниринг промптов = подпроцесс (а не операция и не тип)

```
Процесс: «Разработка AI-решения»
    ↓
Подпроцесс: «Инжиниринг промптов» (Prompt engineering)
    ↓
Операции:
    - Анализ задачи для промпта   (prompt task analysis)
    - Написание черновика промпта (draft writing)
    - Тестирование промпта        (prompt testing)
    - Итерация промпта            (iteration)
    - Валидация промпта           (prompt validation)
```

Инжиниринг промптов — это **подпроцесс** (набор операций с собственными
артефактами), а не одиночная операция и не тип требования. Это согласуется с
триадой Вигерса **Процесс → Операция → Артефакт**.

### Разделение артефактов промптов

Один «промпт» — это на деле **два разных артефакта** с разным назначением и местом
хранения:

| Артефакт | Тип | Пример пути | Назначение |
| --- | --- | --- | --- |
| **Исполнимый промпт** (`executable prompt`) | Исполнимый | `prompts/analyze-doc.md` | Текст, который **запускается** в LLM (Tool). |
| **Prompt specification** (`спецификация промпта`) | Описательный | `docs/prompts/analyze-doc-spec.md` | Описание промпта: назначение, входные данные, ожидаемый выход, используемый контекст/знания, критерии качества, примеры использования и **связь с бизнес-требованиями**. |

> 📎 **Обоснование.** `Prompt specification` отделён от исполнимого промпта, потому
> что он нужен для **отладки** промптов и создания **guides** (документации): по
> нему понятно, что промпт делает, зачем, как его тестировать и с какими
> требованиями он связан, — без необходимости читать и реверсить сам исполнимый
> текст. Исполнимый промпт меняется быстро; спецификация фиксирует контракт.

### Обновлённая сравнительная таблица (куда что относится)

| Колонка | Было (PR #248, ошибочно) | Стало (исправлено) |
| --- | --- | --- |
| **Типы требований** | Функция / Качество / Ограничение / Бизнес-правило **+ Prompt engineering** ❌ | Функция / Качество / Ограничение / Бизнес-правило (Prompt engineering **убран**). |
| **Подпроцессы** | — | **+ Инжиниринг промптов**, Настройка RAG, Оркестрация агентов (см. ниже). |
| **Артефакты** | SRS / Use Case / User Story | **+ Prompt specification**, Исполнимый промпт, Agent architecture document, RAG configuration document. |

## Процессы и подпроцессы для разработки AI-агента (база Фазы 2)

> 🏗️ **Назначение раздела.** Это фундамент в Mango для плавного перехода от
> эксплуатации отдельных промптов (Фаза 1) к внедрению **мультиагентных систем**
> (Фаза 2). Процесс ниже размечен как `Candidate` (доверие E1–E2): он опирается на
> практику AI-разработки (LangChain, CrewAI), но нормой станет только после
> воспроизводимого подтверждения. Поток: «От идеи до AI-агента».

**Процесс: «Разработка AI-агента»** (`AI agent development`) — состоит из пяти
подпроцессов, каждый со своими операциями и артефактом:

### Подпроцесс 1: «Проектирование архитектуры агента» (`agent architecture design`)

- Операция 1.1: «Определение ролей агента» (`role definition`)
- Операция 1.2: «Проектирование workflow агента» (`workflow design`)
- Операция 1.3: «Определение инструментов агента» (`tool definition`)
- **Артефакт:** `Agent architecture document` (документ архитектуры агента)

### Подпроцесс 2: «Настройка RAG» (`RAG configuration`)

- Операция 2.1: «Выбор векторной базы» (`vector store selection`)
- Операция 2.2: «Настройка chunking» (`chunking configuration`)
- Операция 2.3: «Настройка retrieval» (`retrieval configuration`)
- **Артефакт:** `RAG configuration document` (документ конфигурации RAG)

### Подпроцесс 3: «Инжиниринг промптов» (`prompt engineering`)

- Операция 3.1: «Анализ задачи для промпта» (`prompt task analysis`)
- Операция 3.2: «Написание черновика промпта» (`draft writing`)
- Операция 3.3: «Тестирование промпта» (`prompt testing`)
- Операция 3.4: «Итерация промпта» (`iteration`)
- Операция 3.5: «Валидация промпта» (`prompt validation`)
- **Артефакты:** `Prompt specification` + `Исполнимый промпт` (executable prompt)

### Подпроцесс 4: «Оркестрация агентов» (`agent orchestration`)

- Операция 4.1: «Определение сценариев взаимодействия» (`interaction scenarios`)
- Операция 4.2: «Проектирование протоколов коммуникации» (`communication protocols`)
- Операция 4.3: «Настройка координации» (`coordination configuration`)
- **Артефакт:** `Agent orchestration diagram` (диаграмма оркестрации агентов)

### Подпроцесс 5: «Тестирование AI-компонентов» (`AI component testing`)

- Операция 5.1: «Написание тестовых сценариев» (`test scenario writing`)
- Операция 5.2: «Запуск тестов» (`test execution`)
- Операция 5.3: «Анализ результатов» (`results analysis`)
- **Артефакт:** `AI test suite` (набор тестов AI-компонентов)

> 🧭 **AI-assisted ≠ «кнопка».** Переход классической операции в AI-версию
> (например, «Анализ документов» → «AI-assisted document analysis») — это
> **осознанный переход**, а не простое «включение кнопки». Он требует обучения БА
> работе с AI-инструментами, настройки инструментов, валидации AI-выводов и
> интеграции в роадмап. Поэтому AI-операции вводятся пофазно, под реальную боль.
...
- Классификация AI-разработки исправлена: prompt engineering убран из типов
  требований; зафиксированы Prompt = интерфейс к Tool, инжиниринг промптов =
  подпроцесс, разделение `Исполнимый промпт` / `Prompt specification`.
- Добавлен раздел «Процессы и подпроцессы для разработки AI-агента» (процесс
  «Разработка AI-агента» + 5 подпроцессов с операциями и артефактами) как база
  Фазы 2.
...
4. **Делать ли С4/С5 сейчас** или отложить до возникновения конкретной боли
   (Anti-Inflation)?
5. **Принять ли исправленную классификацию AI-разработки** (Prompt = интерфейс к
   Tool; инжиниринг промптов = подпроцесс, не тип требования; разделение
   `Исполнимый промпт` / `Prompt specification`) как рабочую модель Mango для
   Фазы 2, со статусом `Candidate` (E1–E2) до воспроизводимых подтверждений?
